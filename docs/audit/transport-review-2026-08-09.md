...
дефект. Более ранний прогон того же дня давал «требует внимания» в 2 проверках из 9 —
состояние оказалось преходящим (тот прогон шёл во время сетевой возни агентов).
...
* Остальное (догон сделок по `fromId`, глобальная пауза 429, live-добор по суткам из
  `missing`, `fetch_server_ms` без try) — предложения с адресами в §5, каждое трогает
  больше одного модуля и заслуживает отдельной правки с контролем.
