...
## 7. Три оставшихся хвоста ИСПОЛНЕНЫ 2026-08-10

Закрыты все три предложения, отложенные 09.08 как «трогают больше одного модуля».
Расчёта ни одно не касается — это транспорт и расписание, поэтому дифф повтора здесь
ничего не показал бы; вместо него контроль пробником с подсаженными условиями.

Воспроизведение (сеть не нужна, биржевые методы подменены заглушками):

```bash
uv run python docs/audit/probes/probe_transport_tails_2026-08-10.py
```

**1. Догон потерянных сделок по `fromId`** (предложение №1, названо «самым крупным
пробелом»). `Exchange.fetch_agg_trades_from(symbol, from_id, upto_id)` добирает ровно
номера разрыва курсором `fromId`; `_watch_trades_impl` снимает границы разрывов ДО
учёта партии (иначе `seq.note` уже сдвинул курсор) и доливает добранное в `hist` и
`binned` теми же `add`. Предел `TRADE_RECOVER_MAX_IDS = 3000` — до 3 запросов на
событие; вес aggTrades 20 за запрос **по документации Binance, нами не замерен**
(в отличие от klines, где замер есть), фактическое потребление видно в `weight_peak`.

⚠ Счётчики потери потока (`trade_gaps`, `trade_gap_events`) добор НЕ обнуляет: они
меряют потерю ВЕБСОКЕТОМ, и компенсация другим источником этого факта не отменяет.
Добранное и недобранное считаются отдельно (`trade_gaps_recovered` /
`trade_gaps_unrecovered`) и печатаются в приёмке строкой ниже разрывов.

Контроль (прибор доказуемо отвечает по-разному): точный разрыв 100–110 даёт ровно 10
сделок, хотя заглушка отдаёт страницу С ХВОСТОМ за границей — хвост отброшен; широкий
разрыв 0–5000 срезан пределом до 3000; пустой ответ биржи даёт 0 и НЕ выдаётся за
добор; на подсаженном разрыве 4–6 гистограмма получает 8 сделок при доборе и 5 при
отказе REST, причём `trades_unrecovered` растёт до 3 именно в случае отказа.

**2. Глобальная пауза 429 вместо пер-задачной** (предложение №3). На объекте
`Exchange` метка `_rest_quiet_until_ns` по МОНОТОННЫМ часам (тишина нужна и до
сведения часов); `_declare_quiet` только удлиняет — короткая пауза не укорачивает уже
объявленную; `_quiet_gate()` стоит в начале каждого REST-метода класса
(`fetch_server_ms`, `reload_markets`, `_fetch_ohlcv_guarded`, `count_history`,
`fetch_agg_trades_from`). Обе ветки лимита — REST и WS — объявляют тишину: окно веса
у биржи одно на IP, и опрос баров бьёт в него так же, как повтор подписки.
Придержанные вызовы считаются (`rest_gate_held`) — без знаменателя «пауза объявлена»
неотличимо от «пауза никого не остановила».

Контроль: без объявленной тишины ворота пропускают за 0 мс, после `_declare_quiet(1.0)`
держат 1.00 с, вторая тишина 0.2 с первую не укорачивает, `rest_gate_held` = 1.

**3. `fetch_server_ms` без try** (предложение №6). Обёрнут ВЫЗЫВАЮЩИЙ в пакетном
пути — `run.collect`: повторный замер часов после сбора ронял `ccxt.BaseError` весь
прогон уже ПОСЛЕ удавшегося сбора данных. Теперь отказ считается
(`clock_resync_failures`), прежнее сведение остаётся, прогон доходит до карточек.
Служебный путь (`_resync_clock_impl`) был обёрнут с 05.08 — расхождение двух путей и
было дефектом.
