...
Пределы прибора (вердикт falsifier 2026-08-18, контроли — tg-widths-controls-*.py):
- потолок 60% по построению: зону шире прибор назвать не может (подсадка 100%
  глотается фильтром); все 6 реальных отбросов проверены поштучно — не зоны;
- «размах всех чисел» тянет в зону ключевые уровни и цели: по строгому критерию
  (только пара X–Y) медианы ниже на треть — оба критерия печатает контрольный зонд;
- тикер = первый хештег сообщения: в мульти-активных постах пофамильная привязка
  может врать (id 7660 — три актива); класс BTC/ETH-vs-альты устойчив (перестановка);
- строки без пяти эмодзи-меток и без «диапазон интереса» не осматриваются
  (64 строки с парой чисел из 227 — вне обзора).
"""
import re
import statistics
...
src = Path("prizzrak-tg/prizrak_tg_flat.txt")  # запуск из корня репозитория
...
print(f"дециль 10% {qs[0]:.2f} | квартиль 25% {qs[1] if False else statistics.quantiles(ws, n=4)[0]:.2f} "
      f"| медиана {statistics.median(ws):.2f} | 75% {statistics.quantiles(ws, n=4)[2]:.2f} | 90% {qs[8]:.2f}")
...
for name, arr in (("BTC/ETH", maj), ("альты", alt)):
    ...
    if a:
        print(f"\n{name}: n={len(a)}, медиана {statistics.median(a):.2f}%, "
              f"25% {statistics.quantiles(a, n=4)[0]:.2f}%, 75% {statistics.quantiles(a, n=4)[2]:.2f}%, "
