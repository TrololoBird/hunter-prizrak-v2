...
# (⚠ исправлено 2026-08-09: прежний grep считал только «готов» и не видел «закрыт …» —
# показывал 4 разобранных при фактических 8)
grep -cE '\| (готов|решение владельца|\*\*закрыт)' docs/audit/survey-catalog.md; grep -c '| не начат' docs/audit/survey-catalog.md
```
...
| 8 | уровни BUY/SELL | `src/hunter/levels.py` | support resistance detection, price level clustering, level strength scoring, supply demand zones, order block detection | **закрыт 2026-08-10**, 20 реализаций | [levels-projects-2026-08-09.md](levels-projects-2026-08-09.md): **курс отвечает на 11 из 11** — единственный такой элемент, часть Б шкалы пуста и гипотез НОЛЬ; внесена У3 (сетка профиля по размаху баров, сделок 9→4), У6 за владельцем; попутно закрыт НЕДЕТЕРМИНИЗМ карточки |
| 9 | приоритет таймфреймов | `src/hunter/priority.py` | multi timeframe confluence, higher timeframe bias, mtf alignment, **HTF trend filter, top-down analysis** | **закрыт 2026-08-10**, 20 реализаций | [priority-projects-2026-08-10.md](priority-projects-2026-08-10.md): курс отвечает на 9 из 10, следуем на 7; внесена П5 (условия хеджа стр. 11 дошли до карточки, 41% структур), П7 ОТМЕНЕНА контролем — **третий провал критерия одной формы подряд**, разбор способа вынесен владельцу |
...
| 8 | 42 | 24 | **20** ✅ цель 20 взята | 5 из 11; 3 «мало данных», 3 «почти никто», У9 — **0 из 20** | **0 гипотез** (курс отвечает на всё) + 4 замера цены | **1 из курса** — У3; У6 за владельцем |
| 9 | 24 | 23 | **20** ✅ цель 20 взята | 5 из 10; 2 «мало данных», **2 нуля — и оба там, где курс говорит** | **0 гипотез** (курс молчит по одному вопросу, и там мы с полем) + 3 замера цены | **1 из курса** — П5; П7 отменена контролем |
