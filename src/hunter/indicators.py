...
⚠⚠ В РАСЧЁТЕ УЧАСТВУЮТ ДВА ИЗ ТРЁХ: 12 и 26. Число 9 — период СИГНАЛЬНОЙ линии, а её
не считает никто (`macd_signal` удалена 2026-08-06, разбор — комментарием ниже, у
`macd_line`). Оговорка добавлена 2026-08-24: докстрока перечисляла три числа с тремя
источниками и читалась как «все три в деле», тогда как `MACD_SIGNAL` не читается
ниоткуда. Курс сигнальную линию и не требует — стр. 65 дословно: «Базовое применение
индикаторов RSI/MACD – отслеживание дивергенций/конвергенций», а дивергенция строится
по ЛИНИИ MACD; пересечений курс не называет нигде. Константа оставлена как готовый
период на случай, если пересечения когда-нибудь понадобятся, — но пока она НЕ ДЕЙСТВУЕТ,
и это сказано здесь, а не только внизу файла.
...
def ema(period: int) -> pl.Expr:
    ...
    Цена вопроса ЗАМЕРЕНА (зонд probe_ema_seed_2026-08-09 (удалён 19.08), 318 серий):
