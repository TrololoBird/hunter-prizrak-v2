...
BARS_ON_CHART = 180
"""Ширина графика в барах — И РАМКА КАДРА ДЛЯ ВСЕГО ПРОЕКТА.

⚠ Определение переехало сюда из `tgbot` 2026-08-19. Причина не в порядке: этой рамкой
пользуются ДВОЕ — рисование (ширина картинки); доставка пользовалась до
2026-08-21 (`near_structure`: уровень со
структурой старше кадра из сводки скрыт) и СЛУЖБА (не строить профиль объёма тому, кого
ни один из первых двух показать не может). `tgbot` импортирует `service`, поэтому
обратный импорт замкнул бы круг, а вторая копия числа разошлась бы с первой при первой
же правке — тот же дефект, что «две сущности под одним именем».
...
class ZoneSpec:
    ...
    vrvp_density: float = 0.0
    """Плотность объёма в зоне: во сколько раз объём на ценовую строку профиля выше
    среднего по композиту. Ноль — не считалось (строка карты до схемы 8). Вторая
    половина критерия силы: стр. 22 — «Сила уровня определяется ТФ и объемом».
