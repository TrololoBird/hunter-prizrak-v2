...
def resolve(trends: dict[str, Trend], above: str) -> Priority:
    ...
    # ⚠⚠⚠ БЛИЖАЙШИЙ СТАРШИЙ, А НЕ САМЫЙ СТАРШИЙ. Правка 2026-08-28 по приказу владельца
    # «перейди на классику в обоих». Здесь стояло `for tf in reversed(higher)` — обход
    # СВЕРХУ ВНИЗ, то есть приоритет брался с самого старшего определившегося ТФ.
    #
    # Стр. 47 озаглавлена «ПРИОРИТЕТ СТАРШИЙ ТАЙМФРЕЙМ» и КАКОЙ именно старший не
    # уточняет. Классика уточняет: тройной экран Элдера ставит между экранами множитель
    # 4-6 и берёт тренд с ОДНОЙ ступени вверх — "higher timeframe for trend
    # identification, intermediate for entry timing, lower for execution". Наша лестница
    # и есть такие ступени: 5м→15м ×3, 15м→1ч ×4, 1ч→4ч ×4, 4ч→1Д ×6, 1Д→1н ×7.
    #
    # ЦЕНА, замер на кадрах `zonesplit` (138 уровней) ДО правки: приоритет брался с
    # недельного ТФ у 110 из 138 (80%) — пятиминутная сделка судилась недельным трендом.
    # Два прочтения расходились по ТФ у 73 сетапов, и у 34 из них (29% всех) направление
    # было ПРОТИВОПОЛОЖНЫМ, то есть вердикт «по тренду / ПРОТИВ тренда» переворачивался.
    # Разобранный случай: BCH 5м — самый старший (1н) вниз, ближайший старший (15м) вверх.
    #
    # Пропуск ТФ без тренда сохранён: «ближайший» означает ближайший ОПРЕДЕЛИВШИЙСЯ, иначе
    # молчание одного экрана обнуляло бы приоритет целиком.
    for tf in TF_ORDER[TF_RANK[above] + 1:]:
        t = trends.get(tf)
