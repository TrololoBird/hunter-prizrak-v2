...
def _detect_side(
    ...
    for i in range(1, len(others)):
        ...
        # (перемер со свидетелем probe-samebar-2026-08-17, удалён 19.08). Такой лой стоит
...
def current_splits(pps: Sequence[Pereprior]) -> tuple[SplitEntry, ...]:
    """Пары, действующие СЕЙЧАС: не больше одной на сторону.

    ⚠⚠ ЗАВЕДЕНО 2026-08-23 ПО ЖАЛОБЕ ВЛАДЕЛЬЦА НА КАРТОЧКУ: «исправь дублирование строк
    ПП». Дублирования в буквальном смысле не было — все строки несли РАЗНЫЕ зазоры и
    разное число баров между подтверждениями, — но читались они как повтор, и правильно
    читались. Карточка печатала `split_entries` ЦЕЛИКОМ, то есть КАЖДУЮ пару за всю
    историю ряда: живой прогон SOL 2026-08-23 дал **394 строки** в одном разделе, из них
    215 на 5м, 90 на 15м, 39 на 1ч, 33 на 4ч, 14 на 1Д, 3 на 1Н.

    ДЕФЕКТ БЫЛ В РАЗНОБОЕ МЕРОК, А НЕ В ЧИСЛЕ СТРОК. Рядом, в том же разделе, сами
    переприоры печатались по ДРУГОМУ правилу — `SeriesRead.perepriors`, то есть ПОСЛЕДНИЙ
    ПП каждой стороны. Две разные величины под одним человеческим именем «переприор»
    ровно там, где владелец читает их подряд.

    ЧТО ВЗЯТО ЗА «СЕЙЧАС» И ПОЧЕМУ БЕЗ ПОРОГА. Стр. 51 — «Иногда бывает истинный ПП и
    ранний ПП +- рядом, тогда закуп лучше делить на 2 части» — советует про ТЕКУЩИЙ
    закуп. Значит пара печатается тогда и только тогда, когда в неё входит тот самый ПП,
    который карточка показывает строкой выше: последние ДВА ПП стороны, и только если
    они разного вида. Отсечки по расстоянию по-прежнему НЕТ (курс её не даёт), зазор
    по-прежнему печатается числом — сузился не критерий близости, а окно истории.

    ⚠ `split_entries` НЕ ТРОНУТА: она остаётся честным примитивом «все соседние пары»
    и переиспользуется здесь на срезе из двух ПП, чтобы арифметика зазора не разошлась
    во второй копии.
    """
    out: list[SplitEntry] = []
    for side in (PPSide.SHORT, PPSide.LONG):
        seq = sorted((p for p in pps if p.side is side),
                     key=lambda p: p.confirmed_at_index)
        if len(seq) < 2 or seq[-1].kind is seq[-2].kind:
            continue
        out.extend(split_entries(seq[-2:]))
    return tuple(out)
