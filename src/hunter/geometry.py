...
def build_targets(level: Level, pool: tuple[MappedLevel, ...]) -> tuple[Target, ...]:
    ...
    for mapped in pool:
        ...
        if step <= -2:
            ...
            # цены. По ATR своего ТФ та коробка нормальна (недельная волатильность актива
            # огромна), поэтому порог `levels.MAX_BASE_ATR` её и не тронул — ограничение
            # нужно здесь, у ВЫБОРА ЦЕЛИ, а не у построения уровня.
            #
