...
class TargetRole(StrEnum):
    ...
    ⚠ ЗДЕСЬ СТОЯЛО «`build_targets` эту роль не порождает — он работает по уровням пула;
    её ставит сделка от стопового объёма (стр. 39)». Верно до 2026-08-24: с этого дня
    роль порождает и обычная сделка от уровня, потому что стр. 19 называет тейк по
    встречной границе ПЕРВЫМ и описывает им сделку от уровня, а не от стопового объёма.
    Разбор и цена молчания — в `build_targets`."""
...
def take_share(role: TargetRole, *, first: bool = True) -> TakeShare:
    ...
    if role is TargetRole.BOUNDARY:
        # ⚠ ДВЕ СТРАНИЦЫ, А НЕ ОДНА (правка 2026-08-24). Нота ссылалась только на
        # стр. 39, потому что роль порождала одна лишь сделка от стопового объёма.
        # У обычной сделки то же место называет стр. 19: «По верхней границе – делаете
        # тейк 50% - но не 100%». Числа не спорят — 50% это верхний край диапазона
        # 30-50%, — но ссылка на стр. 39 в карточке обычной сделки отсылала владельца
        # не к тому правилу.
        return TakeShare(min_pct=BOUNDARY_TAKE_MIN_PCT, max_pct=BOUNDARY_TAKE_MAX_PCT,
                         note="стр. 19: «По верхней границе – делаете тейк 50% - но не "
                              "100%»; стр. 39: «цель - верхняя граница, где кроем часть "
                              "30-50%»")
    if not first:
        return TakeShare(min_pct=None, max_pct=None,
...
def build_targets(level: Level, pool: tuple[MappedLevel, ...]) -> tuple[Target, ...]:
    ...
    # ⚠⚠ ВСТРЕЧНАЯ ГРАНИЦА СВОЕЙ ЖЕ БАЗЫ — ЦЕЛЬ, И КУРС НАЗЫВАЕТ ЕЁ ПЕРВОЙ (2026-08-24).
    # Стр. 19 разбирает сделку от уровня целиком и ровно тремя шагами: «набираете позицию
    # в Лонг у нижней границы. Цена показала реакцию и ушла внутрь базы – ставите стоп в
    # б/у. По верхней границе – делаете тейк 50% - но не 100%». Тейк назван у ГРАНИЦЫ
    # СОБСТВЕННОЙ базы, а не у чужого уровня из пула, и до этой правки он не строился
    # НИГДЕ: `build_targets` обходил только пул, а роль `TargetRole.BOUNDARY` порождала
    # одна лишь сделка от стопового объёма (`build_stop_volume_setup`, стр. 39).
    #
    # Цена молчания замерена на СВЕЖИХ данных 2026-08-24 (Crypto.com, 23 символа,
    # ТФ 15м/1ч/4ч по 300 баров): у 75% эмитируемых сделок целей не было ВООБЩЕ — ни
    # одной. Такая сделка не просто «без ориентира»: `outcome.resolve` с `target=None`
    # физически не может вернуть `target`, то есть положительного R у неё не бывает по
    # построению, и в статистику леджера она входит как заведомый не-выигрыш.
    #
    # Условие «впереди входа» — то же, что у стр. 39 (`build_stop_volume_setup`): если
    # ПОК стоит на самой границе или за ней, цели нет, и подставлять её нельзя.
    goal = level.boundary_hi if up else level.boundary_lo
    if (goal > entry) if up else (goal < entry):
        out.append(Target(
            role=TargetRole.BOUNDARY, price=goal, timeframe=level.timeframe,
