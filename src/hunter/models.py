...
class Bar:
    ...
    ⚠ Числа строки dataclass ОТОЗВАНЫ 2026-08-21 пересчётом tracemalloc: 244 байта на
    бар (см. `config.py`, там же команда), производная «0.23 ГБ» занижена вдвое.
    Кратность против pydantic сохраняется, вывод решения не меняется. Вторая копия
    числа снята ревизией 2026-09-01 (правило «величина считается в одном месте»).
...
class SeriesState(BaseModel):
    ...
    decided_depth: int = 0
    """Глубина ряда, РЕШЁННАЯ засевом для этого ТФ (`run.seed_depth`). Ноль — не
    записана (кадры до 2026-08-27), тогда ряд идёт в расчёт целиком, как раньше.

    ⚠⚠ ЗАВЕДЕНО 2026-08-27 ПО НАЙДЕННОМУ ДЕФЕКТУ. `seed_depth` объявлен «единственным
    местом, где решается глубина ряда», но решал он лишь РАЗМЕР ЗАПРОСА к бирже: в
    расчёт шёл накопленный кэш целиком. Замер: два прогона, `--seed-limit 300` и
    `--seed-limit 200`, дали в кадрах 15м по 1629 и 1630 баров, и выдача совпала почти
    до цифры — то есть ключ обещал глубину и её не менял. Хуже того, это нарушало
    правило свода «разметка не смеет быть функцией того, СКОЛЬКО баров скачали»:
    длина ряда решения определялась историей закачек, а не объявленным окном."""
...
class RunReport(BaseModel):
    ...
    break_signals_recorded: int = 0
    break_signals_known: int = 0
    break_signals_no_target: int = 0
    break_signals_far: int = 0
    """СДЕЛКИ ПО СЛОМУ младшего ТФ — стр. 25/31 разрешают вход, стр. 50 говорит какой.

    ⚠ Заведено правкой A-3, 2026-08-28, по решению владельца. Род сигнала тот же (`pp`):
    слом структуры и есть переприор (стр. 49), и торгуется он тестом ПП. Считается
    ОТДЕЛЬНО от самостоятельных сделок от ПП, потому что это разные ПОВОДЫ: там ПП сам
    по себе, здесь — ПП, подтвердивший вход от конкретного уровня. Сумма в одном числе
    прятала бы перекос, ради которого счётчики и разделены.

    ⚠ ДВА СЧЁТЧИКА ОТКАЗОВ ОБЯЗАТЕЛЬНЫ (§4.3): без них «сделок по слому 0» читается как
    «сломов не было», тогда как причин три и они разные — слома действительно нет, у
    сделки нет цели (неизмерима), цена далеко от зоны теста.
    """
    ...
    no_target_by_sink: dict[str, int] = Field(default_factory=dict)
    """ВОРОНКА ЦЕЛЕЙ: сколько эмиссий лишились КРУПНОЙ цели на каждом фильтре.

    ⚠⚠ ЗАВЕДЕНО 2026-08-26, Ф1 плана, и заведено как ЗНАМЕНАТЕЛЬ к `emitted_rr`. Раньше
    длина `emitted_rr` была короче числа эмиссий, и разница нигде не объяснялась: у
    сделки без крупной цели РР не существует (`Setup.rr` → None), она молча выпадала из
    списка, и раздел 12 приёмки печатал «РР ... из N» по подвыборке, которую никто не
    называл. Причина у каждой такой сделки была — но жила СТРОКОЙ в карточке, то есть
    существовала для человека и не существовала для прогона.

    Цена молчания: долю сделок без крупной цели дважды считали разовым замером и
    получили 96.6% (прошлая смена, по эмитируемым) против 52.5% (по всем размеченным).
    Оба числа правдоподобны, сложить их было нечем, и какое из них про боевой прогон —
    неизвестно до сих пор. Теперь это число печатается каждым прогоном.

    Ключи — значения `geometry.TargetSink`, а не свободный текст: печать обходит
    перечисление, поэтому новый сток появляется в сводке сам, а не забывается (тот же
    приём, что у `emitted_outcomes`).

    ⚠ КОНТРОЛЬ СХОДИМОСТИ, который обязан держаться на любом прогоне:
    `sum(no_target_by_sink.values()) + len(emitted_rr) + <вырожденный риск при цели>`
    равно числу эмиссий. Расхождение означает потерянную строку — ровно тот дефект,
    ради которого поле и заведено.

    ⚠ Ненулевой сток — НЕ ДЕФЕКТ САМ ПО СЕБЕ: часть стоков методическая (уровень снят
    курсом, стр. 25), часть — отказ прибора (структура старшего ТФ не построилась).
    Разделить их может только человек с референтом. Ослаблять фильтр, чтобы «появились
    цели», запрещено: порог на выходе не лечит расчёт на входе, и в обратную сторону
    правило работает так же.
    """
    ...
    levels_box_to_base: list[float] = Field(default_factory=list)
    """Отношение сетки профиля к границам базы, по уровню (`Level.box_to_base`).

    ⚠ Заведено 2026-08-27. Внешний референт назван числом: у автора на рисунке стр. 30
    это отношение 1.31 (пиксельный замер). Медиана нашей выдачи и есть мера того,
    насколько детектор рисует рамку не там; отказ `ZONE_OUT` — её симптом."""

    levels_volume_outside_base: list[float] = Field(default_factory=list)
    """Доля объёма ВНЕ базы по каждому уровню (`Level.volume_outside_base`).

    ⚠ Список, а не счётчик: у величины важен ХВОСТ. Медиана 11% и максимум 95% — разные
    вести, и одно число их не различает."""

    levels_zone_one_row: int = 0
    """Уровни, у которых ВСЯ область стоимости уместилась в одну-две строки профиля.

    ⚠ Заведено 2026-08-27 по находке ГЛАЗАМИ в карточке BTC: уровень 15м с базой
    0.127% цены и зоной шириной ровно в одну строку. У такой зоны разрешения нет
    вовсе — VAL, VAH и ПОК совпадают, и «зона» перестаёт отличать что-либо от чего-либо.
    Замер по 20 символам Binance: 1 из 542 (0.2%), и тот уже перевёрнут. Порога здесь
    НЕ вводится: пока не показано, что вход считается верно, фильтр был бы сокрытием
    (правило свода «порог на выходе не лечит расчёт на входе»). Число называется, чтобы
    рост был виден.
    """

    levels_work_zone_not_narrowed: int = 0
    """Уровни, у которых рабочая зона вокруг ПОК совпала со всей VAL…VAH.

    ⚠ Заведено 2026-08-27. Владелец 2026-08-25 просил «вокруг пока строить небольшую
    зону»; замер по 20 символам Binance: у 130 уровней из 542 (24.0%) HVN-ядро не сузило
    НИЧЕГО, медиана сужения 0.67. То есть у четверти уровней «вход по ядру» и «вход по
    всей области стоимости» — одно и то же, и решение владельца там не исполняется.
    """

    unbuilt_by_kind: dict[str, int] = Field(default_factory=dict)
    """Структур, не давших уровня, ПО КЛАССУ причины (`levels.UnbuiltKind`).

    ⚠⚠ ЗАВЕДЕНО 2026-08-26 РАДИ Ф7, и без него фаза неисполнима: она требует «долю
    структур, не давших уровня ПО ПРИЧИНЕ ПОКРЫТИЯ, за неделю боевых прогонов», а причина
    существовала свободной СТРОКОЙ — с символом и ценами внутри, — и две одинаковые по
    смыслу причины были разными строками. Сложить их было нечем ни за неделю, ни за день.

    ⚠ Не всякий класс — отказ. `FAR` и `DUPLICATE` суть ФИЛЬТРЫ: уровень построен и снят
    нарочно. Складывать их с отказами снабжения в одно число «потеряно» — соврать."""

    unbuilt_coverage_by_tf: dict[str, int] = Field(default_factory=dict)
    """ЧИСЛИТЕЛЬ МЕРЫ Ф7: структур, не давших уровня из-за ПОКРЫТИЯ, по ТФ."""

    structures_by_tf: dict[str, int] = Field(default_factory=dict)
    """ЗНАМЕНАТЕЛЬ МЕРЫ Ф7: структур разобрано по ТФ — построенные уровни плюс все
    непостроенные. Без него числитель не читается: десять отказов при десяти структурах и
    при тысяче — разные утверждения."""

    profile_windows_no_bars_by_tf: dict[str, int] = Field(default_factory=dict)
    """Окон профиля, НЕ ПОКРЫТЫХ свечами вовсе, по ТФ разрешения. «Период не качали»."""

    profile_windows_partial_by_tf: dict[str, int] = Field(default_factory=dict)
    """Окон профиля, покрытых НЕ ПОЛНОСТЬЮ, по ТФ разрешения (Ф6, 2026-08-26).

    ⚠⚠ ЭТОТ КЛАСС РЕЖЕТ УРОВНИ, И ДО ЭТОГО ДНЯ ОН НЕ СВОДИЛСЯ НИ ПО КАКОМУ ИЗМЕРЕНИЮ.
    Отказ считался внутри `profile_source.CandleWindows` и наружу не выходил: в отчёт шёл
    только третий класс — «ряда нужного ТФ не собрано». Правило сводки отказов требует
    обратного: сто отказов, все на одном ТФ, есть дефект, а не рынок, — и увидеть этот
    перекос было НЕЧЕМ.

    ⚠ Лечится он ПРОТИВОПОЛОЖНО соседнему классу: пустое окно означает, что период не
    качали, а неполное — что качали и не докачали. Поэтому две строки, а не одна."""
    ...
    ⚠ Считает ТОЛЬКО дорешанные сигналы (`run._resolve_pending`) — так эта величина и
    названа. Свежая эмиссия от уровня попадает в `emitted_outcomes` своим членом
    перечисления, а от ПП — в `pp_degenerate_risk`, заведённый рядом."""

    pp_degenerate_risk: int = 0
    """То же у сделки от ПП (стр. 50): стоп совпал со входом, исход в R не измерим.

    ⚠ Заведено 2026-08-26 ревизией слитых PR #4/#5. Вырожденный риск перехватывался в
    ТРЁХ местах `run`, и каждое распоряжалось им по-своему: дорешивание считало
    (`pending_degenerate_risk`) и печатало в приёмке; свежая сделка от уровня попадала в
    `emitted_outcomes` и была видна в разделе 12; а ПП-сигнал не увеличивал НИ ОДНОГО
    счётчика — ни исхода, ни состояния, ни отказа. То есть сделка исчезала из отчёта
    целиком, и приёмка, называя вырожденный риск числом, объявляла охват больше
    настоящего.

    Ноль — законное состояние. Ненулевое означает, что у сделки от ПП стоп встал ровно
    на цену входа: `build_pp_setup` кладёт стоп за край зоны с запасом, поэтому такое
    совпадение — дефект геометрии ПП, а не свойство рынка."""
    ...
    signal_far_from_price: int = 0
    """Сделок НЕ ЗАПИСАНО, потому что цена ещё не пришла к их зоне входа.

    ⚠⚠ ЗАВЕДЕНО 2026-08-27 ВМЕСТЕ С САМИМ УСЛОВИЕМ. Молчание обязано называться числом:
    без этого счётчика «сигналов мало, потому что рынок тихий» и «сигналов мало, потому
    что мы их перестали писать» выглядели бы одинаково. Мера близости — ширина зоны
    (`geometry.zone_position`), та же, которой молчит слой доставки.

    Уровень при этом ОСТАЁТСЯ в карте: режется запись СДЕЛКИ, а не разметка."""

    map_added: int = 0
    map_updated: int = 0
    map_retired: int = 0
    map_unbuilt: int = 0
    """Уровней снято, потому что расчёт их БОЛЬШЕ НЕ СТРОИТ (тоже `stale_calc`).

    ⚠ Отдельно от `map_stale_calc`, хотя состояние в леджере одно: причины РАЗНЫЕ, и
    складывать их в одно число значило бы прятать перекос. Там структура пропала из
    разбора; здесь структура НА МЕСТЕ и внутри горизонта, а уровня по ней не строится —
    чаще всего профиль. Замер 2026-08-28 по трём символам: у всех восьми таких уровней
    структура нашлась в свежем разборе и в горизонте 180 суток.

    ⚠ Заведено 2026-08-28 по приказу владельца «сними 18 несвежих строк с карты». До
    этого такой уровень оставался `active` навсегда: 18 строк из 108.
    """
    ...
    zone_overlap_pairs: int = 0
    """НАСЛОЕНИЕ ВСТРЕЧНЫХ ЗОН в накопленной карте — пар активных уровней
    противоположных сторон с пересечением зон. Разбор — в `store.ZoneOverlaps`.

    ⚠⚠ ЗАВЕДЕНО 2026-08-26. Класс нашёл ВЛАДЕЛЕЦ ГЛАЗАМИ 2026-08-18, чинили его
    ЧЕТЫРЕЖДЫ за неделю — и ни один прогон не назвал числа: прибора для этой величины в
    проекте не было. Замер владельца (5 символов, 4413 активных уровней) давал 385–418
    пар у одного символа; сравнить с ним было нечего до сих пор.

    Ноль — законное состояние. Ненулевое — повод посмотреть, а не отказ: стр. 25
    прямо велит «этот уровень удаляем и ищем новые», то есть встречный уровень на тех же
    ценах бывает и законным. Дефектом является РОСТ числа при неизменном расчёте: он
    означает, что карта снова копит непересуженные уровни."""

    zone_overlap_by_symbol: dict[str, int] = Field(default_factory=dict)
    """Те же пары ПО СИМВОЛАМ — измерение, вдоль которого возможен перекос (правило
    сводки отказов). Двести пар у одного символа и нули у прочих есть дефект символа."""

    zone_overlap_levels: int = 0
    """Активных уровней в карте на момент счёта — ЗНАМЕНАТЕЛЬ к парам. Без него «пар 0»
    у пустой карты неотличимо от «пар 0» у карты в тысячу уровней."""
    ...
    """Рядов, у которых хранилище есть, а свежий хвост не пришёл.

    ⚠ СМЫСЛ ИЗМЕНЁН 2026-08-23 ПО ПРИКАЗУ ВЛАДЕЛЬЦА «дегдрадации, остутсвие данных НЕ
    ДОПУСТИМЫ!». Раньше здесь было: «такой ряд отдаётся старым, и это НАЗВАННАЯ
    деградация». Теперь такой ряд НЕ ОТДАЁТСЯ: он получает `not_ready` и в расчёт не
    идёт, пока опрос не догонит его до последнего закрытого бара (`poll_revived`).
    Счётчик стал мерой того, сколько рядов ушло в ожидание догона, а не сколько рядов
    предъявлено владельцу чёрствыми.
    """

    poll_revived: int = 0
    """Рядов, чей отказ засева СНЯТ опросом: ряд сравнялся с сведёнными часами.

    Пара к `seed_tail_failed` и её знаменатель по смыслу: без этого счётчика «отвергли N
    рядов» неотличимо от «потеряли N рядов». Разница между ними и есть то, что до конца
    прогона в расчёт так и не попало.
    """
    ...
    """Дыр засева, оставшихся открытыми после добора.

    ⚠ ПРИЧИН СТАЛО ТРИ, А НЕ ДВЕ (правка 2026-08-23). Здесь стояло «ровно две: биржа
    отказала (сеть) либо у биржи там баров нет вовсе». Третья найдена живым запросом:
    биржа отдаёт бар, который НЕ ЯВЛЯЕТСЯ свечой, и его отклоняет проверка определения
    (BCH/USDT:USDT 1w, неделя 2020-01-13: максимум 215.69 ниже открытия 339.65). Лечение
    у третьей своё — пересборка из младшего ТФ, счётчик `seed_gaps_rebuilt`, — и потому
    сводить её с молчанием биржи в одно число нельзя: повтор запроса лечит первую и
    вечно топчется на третьей.

    Молчать нельзя ни о какой — иначе повторный запрос того же участка каждый прогон
    выглядел бы работой, а не топтанием."""

    seed_gaps_rebuilt: int = 0
    """Баров, ПЕРЕСОБРАННЫХ из младшего ТФ вместо битого агрегата биржи.

    Знаменатель к нему — `seed_gaps_left`: пересборка пробуется только там, где бар
    пришёл и был отклонён. Ноль здесь при ненулевом `seed_gaps_left` означает, что
    открытые дыры имеют ДРУГУЮ причину, а не что пересборка не работает."""
