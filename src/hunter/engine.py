...
class MtfBreak(StrEnum):
    """Состояние правила стр. 31: «Позицию от уровня смотрим только по факту слома
    структуры на более мелких ТФ».

    ⚠⚠ ЗАВЕДЕНО ПРАВКАМИ A-2 и A-5, 2026-08-28. До них ответ существовал ТОЛЬКО русской
    фразой, а `run.py` сводил её к двум значениям поиском подстроки «подтверждён». В ноль
    сваливались три РАЗНЫЕ вещи: «слома не было», «ряд не разобран» и «младшего ТФ у
    курса нет». Цена измерена по активной карте: 28 уровней 5м ждали слома, которого
    смотреть негде, и 10 из них входили ТОЛЬКО по нему — то есть не активировались бы
    НИКОГДА, оставаясь в карте годными ко входу.

    §4.3: отсутствие НАЗЫВАЕТСЯ, а не подменяется значением. «Проверить негде» и «слома
    не было» — разные ответы, и путать их нельзя ни в леджере, ни в карточке.
    """

    CONFIRMED = "confirmed"
    """Слом на младшем ТФ после касания найден — условие входа стр. 25/31 выполнено."""

    ABSENT = "absent"
    """Слома после касания не было. Настоящий отрицательный ответ: искали и не нашли."""

    NO_TOUCH = "no_touch"
    """Цена к уровню не приходила — слом искать НЕ ОТ ЧЕГО, вопрос ещё не возник."""

    NOT_APPLICABLE = "not_applicable"
    """Младшего ТФ у курса нет: стр. 17 не называет ничего младше 5м.

    Значит правило стр. 25/31 к уровню 5м НЕПРИМЕНИМО, а не «не проверено», и ждать от
    него подтверждения бессмысленно — ждать пришлось бы вечно.
    """

    UNKNOWN = "unknown"
    """Ряд младшего ТФ не разобран — проверить негде. ДЕГРАДАЦИЯ, а не ответ «нет»."""


class Decision(BaseModel):
    """Что система решила по ОДНОМУ размеченному уровню. Одна запись — один уровень.

    ⚠⚠ **`setup` БОЛЬШЕ НЕ ОЗНАЧАЕТ «уровень эмитируется» — правка A-1, 2026-08-28.**
    Здесь стояло «setup есть тогда и только тогда, когда уровень эмитируется». Сетап
    теперь считается У ВСЕХ размеченных уровней, потому что курс разрешает вход и по
    удержанным — просто ДРУГИМ путём (стр. 25, 31, 43, 44), а без сетапа у такой сделки
    нет ни стопа, ни цели. Торгуется ли уровень, отвечает `hold` и только он.

    `hold` называет причину словами со ссылкой на страницу курса (§4.3: отсутствие
    называется, а не подменяется значением).
    """
    ...
    mtf_break_state: MtfBreak = MtfBreak.NO_TOUCH
    """СОСТОЯНИЕ слома на младшем ТФ — то, ПО ЧЕМУ принимаются решения.

    ⚠⚠ ЗАВЕДЕНО ПРАВКОЙ A-5, 2026-08-28. До неё решение принималось поиском подстроки
    «подтверждён» в соседнем поле `mtf_break` — то есть человеческая фраза, которую в
    этом проекте переписывают постоянно, управляла целым числом леджера, а оно —
    объявлением «вход активен». Ложного срабатывания на тот день не было; защиты от него
    не было тоже: достаточно переписать фразу.
    """

    mtf_break: str = ""
    """Тот же ответ СЛОВАМИ — для читателя карточки, и только для него.

    Пусто ровно у одного состояния — `NO_TOUCH`: цена к уровню не приходила, и называть
    там нечего. У остальных — либо описание найденного слома, либо названная причина,
    почему его нет. §4.3: отсутствие называется, а не подменяется пустотой.

    ⚠ РЕШЕНИЯ ПО ЭТОМУ ПОЛЮ НЕ ПРИНИМАЮТСЯ. Оно текст, и переписать его должно быть
    безопасно; для решений есть `mtf_break_state`.
    """
    ...
    def emitted(self) -> bool:
        """Торгуется ли уровень СЕЙЧАС. Решает `hold`, а не наличие сетапа.

        ⚠⚠ Правка A-1, 2026-08-28. Здесь стояло `self.setup is not None`, и это было
        верно ровно до тех пор, пока сетап считался только у эмитируемых. Теперь он
        считается ВСЕГДА (ради стопа у уровней, чей вход курс разрешает другим путём),
        и прежний критерий объявил бы торгуемыми все удержанные уровни разом.

        `hold` для этого и заведён: «Пусто — уровень эмитируется». Одна величина в одном
        месте, а не две под одним смыслом.
        """
...
class SymbolDecision(BaseModel):
    ...
    def emissions(self) -> tuple[emit.Emission, ...]:
        ...
        """
        # ⚠ Критерий — `d.emitted`, то есть пустой `hold` (правка A-1, 2026-08-28). До
        # неё стояло `d.setup is not None`, и это была ТРЕТЬЯ запись одного признака
        # рядом с `emitted` и картой. Проверка `setup is not None` осталась только как
        # сужение типа для mypy: по построению `not hold` влечёт непустой сетап —
        # единственная ветка, обнуляющая сетап, тем же движением ставит `hold`.
        return tuple(
            emit.Emission(level=d.level, setup=d.setup, agreement=d.agreement)
            for d in self.decisions if d.emitted and d.setup is not None
        )
...
def decide(
    ...
    for m in mapped:
        ...
        # ⚠⚠⚠ СДЕЛКА БЕЗ ЦЕЛИ НЕ ЭМИТИРУЕТСЯ — приказ владельца 2026-08-27, дословно:
        # «у сигнала НЕ МОЖЕТ БЫТЬ ЦЕЛИ, если цели нет это критический дефект расчетов,
        # логики, анализа или данных!!!! … не надо придумывать костыли или "максировать"
        # сообщениями нет цели или риск высокий!»
        #
        # Это НЕ порог на выходе и не сокрытие: сделка без цели НЕИЗМЕРИМА — у неё нет
        # исхода, по которому считается R, значит она не может ни подтвердить метод, ни
        # опровергнуть. Раньше такая строка уходила в леджер с `target = NULL` и портила
        # накопленную статистику молча.
        #
        # ⚠⚠ СРАБАТЫВАНИЕ ЭТОЙ ВЕТКИ — НЕНАЙДЕННЫЙ ДЕФЕКТ, А НЕ ОБЪЯСНИМЫЙ СЛУЧАЙ.
        # Здесь стояло «все три объяснены курсом: два `TF_TIER`, один `NOT_AHEAD`».
        # Владелец остановил это прямо: «ты блять опять НАЗЫВАЕШЬ КАКУЮ-то ПРИЧИНУ! НЕТ
        # НИКАКОЙ ПРИЧИНЫ, ТОЛЬКО ОШИБКА В ПРОЕКТЕ!» — и он прав по счёту: те самые три
        # сделки исчезли, как только `build_level` перестал отказывать структурам с
        # зоной внутри прокола (стр. 18, расширение базы). Пустой ярус стр. 24 был не
        # свойством рынка, а следствием того, что уровни выбрасывались: у ETH при 997
        # отказах в карте не было ни одного уровня 1н и ни одного шортового 1Д.
        #
        # После правки на тех же кадрах цель есть у 13 сделок из 13. Ветка остаётся
        # сторожем: если она сработает снова — искать ошибку, а не причину. Число
        # печатает воронка целей (`RunReport.no_target_by_sink`, Ф1) со знаменателем.
        # ⚠⚠⚠ СЕТАП СЧИТАЕТСЯ ВСЕГДА, А НЕ ТОЛЬКО У ЭМИТИРУЕМЫХ. Правка A-1, 2026-08-28.
        #
        # Здесь стояло `None if hold else build_setup(...)`, и это читало `hold` как
        # «сделки не будет». Курс так не говорит НИ В ОДНОМ из пяти случаев удержания —
        # каждый сам называет ДРУГОЙ путь входа. Стр. 31 дословно: «уровень лимитными
        # ордерами больше не торгуем… Позицию от уровня смотрим только по факту слома
        # структуры на более мелких ТФ». Стр. 25 то же о втором-третьем касании, стр. 43
        # о ретесте в другую сторону, стр. 44 о цене внутри зоны.
        #
        # что из чего следует: уровень забран с реакцией → запрещена ЛИМИТКА, а позиция
        # берётся другим входом. НЕ «позиции нет».
        #
        # Цена прежнего чтения, замер по боевому леджеру 2026-08-28: у уровней с правилом
        # входа `confirmation` стоп был у 0 из 33. Контроль, что прибор мог ответить
        # иначе: со стопом 71 уровень, из них с подтверждённым сломом — 0, то есть раскол
        # шёл ровно по правилу входа. Механизм: `store.sync_levels` пишет `stop_price`
        # безусловно, поэтому посчитанный ранее стоп ПЕРЕЗАПИСЫВАЛСЯ пустотой на каждом
        # цикле, а бот подставлял свою формулу без якоря стр. 18 — на разобранном BCH 1ч
        # это давало риск 6.44% вместо расчётных 8.51%.
        #
        # ⚠ БЕРЁТСЯ `m.current`, А НЕ `m.level`. У пробитого уровня сторона
        # противоположна (стр. 43), и сетап на стороне РОЖДЕНИЯ описывал бы другую
        # сделку. У эмитируемых уровней разницы нет: флип всегда даёт `hold`, значит
        # `current is level`, — то есть подстановка ничего не меняет там, где раньше
        # работало, и чинит там, где сетапа не было вовсе.
        # Считанный сетап держится в НЕПУСТОЙ переменной, а `built` — то, что уйдёт в
        # решение: обнулять его при отсутствии целей нужно, а терять посчитанное — нет.
        setup_now = geometry.build_setup(m.current, mapped, as_of_ms=now_ms)
        built: geometry.Setup | None = setup_now
        # ⚠⚠⚠ СДЕЛКА БЕЗ ЦЕЛИ НЕ ЭМИТИРУЕТСЯ — приказ владельца 2026-08-27, дословно:
        # «у сигнала НЕ МОЖЕТ БЫТЬ ЦЕЛИ, если цели нет это критический дефект расчетов…
        # не надо придумывать костыли или "максировать" сообщениями нет цели».
        #
        # ⚠ Условие `not hold` обязательно: без него отсутствие целей ЗАТИРАЛО БЫ уже
        # названную курсовую причину удержания своей. У удержанного уровня сетап
        # остаётся и с пустым списком целей — он нужен ради СТОПА, а эмиссию он больше
        # не решает.
        if not hold and not setup_now.targets:
            hold = ("целей нет — сделка неизмерима: у неё нет исхода, по которому "
                    f"считается R ({setup_now.no_target_reason or 'причина не названа'})")
            built = None
        mtf_state, mtf_note = _mtf_break(m, series, reads, tfs)
...
def _mtf_break(
    ...
    # ⚠⚠ СТАРШИНСТВО БЕРЁТСЯ ИЗ `TF_RANK`, А НЕ ИЗ СПИСКА ПРОГОНА (правка A-2,
    # 2026-08-28). Здесь стояло `timeframes.index(tf) == 0` — «младший в ЭТОМ прогоне», —
    # и это была ШЕСТАЯ запись порядка ТФ, ровно та, против которой заведён `TF_RANK`
    # («представлений этого порядка было пять»). Расходились они не теоретически: прогон,
    # настроенный без 5м, объявлял бы 15м-уровню «5м — младший ТФ курса (стр. 17)» —
    # фразой заведомо ложной — и снимал бы с него вход навсегда. Курс называет список
    # ТФ сам (стр. 17), и он от настроек прогона не зависит.
    if tf not in TF_RANK:
        return (MtfBreak.UNKNOWN,
                f"ТФ {tf} не из списка стр. 17 — старшинство не определено")
    if TF_RANK[tf] == 0:
        ...
        return (MtfBreak.NOT_APPLICABLE,
                "5м — младший ТФ курса (стр. 17), слом смотреть негде: "
                "со второго касания уровень не торгуется")
    younger = TF_ORDER[TF_RANK[tf] - 1]
    if younger not in timeframes:
        # ⚠ ЭТО ДЕГРАДАЦИЯ, А НЕ ОТВЕТ. Младший ТФ у курса есть, но в прогон не подан:
        # значит вопрос не задавался, и записывать его в «слома не было» — то самое
        # молчаливое отсутствие данных, которое свод запрещает (§4.3).
        return (MtfBreak.UNKNOWN,
                f"ряд {younger} в прогон не подан — слом проверить негде")
    y_bars = series.get(younger)
    y_read = reads.get(younger)
    own = series.get(tf)
    if not y_bars or y_read is None or not own:
        return (MtfBreak.UNKNOWN, f"ряд {younger} не разобран — слом проверить негде")
    touch = levels.first_test_index(m.level, own)
    if touch is None:
        # Цена к уровню не приходила: слом младшего ТФ у ЭТОГО уровня искать не от чего.
        return (MtfBreak.NO_TOUCH, "")
    ...
    for pp in y_read.all_perepriors:
        ...
        return (MtfBreak.CONFIRMED,
                f"слом на {younger} подтверждён, зона ПП "
                f"{pp.zone_lo:.8g}…{pp.zone_hi:.8g}, "
                f"{'ТЕСТ БЫЛ — ' + why if tested else 'теста ещё не было'}")
    return (MtfBreak.ABSENT,
            f"слома на {younger} у этого уровня после касания не было")
