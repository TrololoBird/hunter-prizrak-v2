...
class Decision(BaseModel):
    ...
    break_setup: geometry.PPSetup | None = None
    """СДЕЛКА ПО СЛОМУ младшего ТФ — стр. 25/31 разрешают вход, стр. 50 говорит КАКОЙ.

    ⚠⚠ ЗАВЕДЕНО ПРАВКОЙ A-3, 2026-08-28, ПО РЕШЕНИЮ ВЛАДЕЛЬЦА. Дословно вопрос ему был
    поставлен так: «реализовать вход по слому как отдельный сигнал или снять из канала
    объявление „вход активен“», и он выбрал реализовать.

    До этого дня расчёт слом ПРОВЕРЯЛ и НАЗЫВАЛ, но сделки по нему не строил: докстрока
    `_mtf_break` прямо говорила «эмиссию НЕ порождает». А бот при этом объявлял в канал
    «слом младшего ТФ подтверждён — вход активен» — то есть звал в сделку, у которой не
    было ни стопа, ни цели, ни РР. 28.08 в 13:03 UTC так ушло по BCH шорт 1ч.

    Своей геометрии здесь НЕ добавляется: слом структуры и есть переприор (стр. 49), а
    торговлю переприора курс описывает целиком — стр. 50: «Торговля ПП – точкой входа
    является тест ПП, со стопом за хай/лой или базу в месте слома, если она есть».
    Значит сделка по слому — это `geometry.build_pp_setup` на МЛАДШЕМ ряде, и в леджер
    она идёт тем же родом `pp`, что и самостоятельные сделки от переприора.

    ⚠ Что из чего следует: уровень отработан или касался (стр. 25, 31) → лимитки сняты
    → вход остаётся ТОЛЬКО по слому структуры на младшем ТФ → слом это ПП (стр. 49) →
    ПП торгуется тестом со стопом за хай/лой (стр. 50). Обратный ход неверен: наличие
    ПП на младшем ТФ само по себе сделки ОТ ЭТОГО УРОВНЯ не даёт — нужно, чтобы ПП
    случился после касания и пересекался с зоной уровня (это проверяет `_mtf_break`).

    `None` — слом не подтверждён либо у сделки нет цели.
    """

    break_pp: Pereprior | None = None
    """Тот самый ПП, которым слом подтверждён. Нужен для записи сигнала: бар его
    подтверждения — момент открытия сделки, а его зона — то, чем меряется приход цены."""

    break_timeframe: str = ""
    """МЛАДШИЙ ТФ, на котором живёт сделка по слому. Не ТФ уровня: тест ПП происходит
    на младшем ряде, и мерить исход надо его барами."""
...
def decide(
    ...
    for m in mapped:
        ...
        if not hold and not setup_now.targets:
            ...
            built = None
        mtf_state, mtf_note, mtf_pp = _mtf_break(m, series, reads, tfs)
        # СДЕЛКА ПО СЛОМУ строится ровно тогда, когда слом ПОДТВЕРЖДЁН, и на МЛАДШЕМ
        # ряде: стр. 50 велит входить тестом ПП, а тест происходит там, где ПП найден.
        # Противоположный ПП для цели берётся из `perepriors` (последний на сторону) —
        # той же выдачей, что у самостоятельных сделок от ПП, чтобы цель одного и того
        # же переприора не считалась двумя способами.
        break_setup: geometry.PPSetup | None = None
        break_tf = ""
        # ⚠⚠⚠ ТОЛЬКО ПРИ ПРАВИЛЕ ВХОДА `CONFIRMATION`, И ЭТО НЕ ПОРОГ, А ОБЛАСТЬ
        # ДЕЙСТВИЯ ПРАВИЛА КУРСА. Вход по слому разрешают стр. 25 и 31, и обе говорят
        # об уровне, ОТРАБОТАННОМ КАСАНИЕМ. Пробитый уровень живёт по стр. 43 — там
        # вход по РЕТЕСТУ и в ПРОТИВОПОЛОЖНУЮ сторону, а не по слому младшего ТФ.
        #
        # Замер первой редакции этой правки (61 сделка, 3 символа): у 29 из 61 (48%)
        # сторона сделки совпадала со стороной РОЖДЕНИЯ уровня и расходилась с той,
        # которой он торгуется СЕЙЧАС, — и все 29 оказались флипнутыми уровнями с
        # правилом `retest_flipped`. То есть проект строил бы сделку в сторону,
        # прямо запрещённую стр. 43, да ещё и называя её исполнением стр. 25/31.
        # Поймано глазами по карточке и перепроверено счётом, а не гейтом.
        #
        # ⚠ `_mtf_break` при этом считается ПО-ПРЕЖНЕМУ у всех уровней: знать, случился
        # ли слом, полезно и у свежего (стр. 19 называет это альтернативой лимитке).
        # Ограничивается СДЕЛКА, а не наблюдение.
        if (mtf_pp is not None
                and m.status.entry_rule is levels.EntryRule.CONFIRMATION
                and TF_RANK.get(m.level.timeframe, 0) > 0):
            break_tf = TF_ORDER[TF_RANK[m.level.timeframe] - 1]
            y = reads.get(break_tf)
            y_bars = series.get(break_tf)
            if y is not None and y_bars:
                opp = next((o for o in y.perepriors if o.side is not mtf_pp.side), None)
                break_setup = geometry.build_pp_setup(mtf_pp, opp, y.scan, y_bars)
...
def _mtf_break(
    ...
    reads: dict[str, SeriesRead],
    timeframes: tuple[str, ...],
) -> tuple[MtfBreak, str, Pereprior | None]:
    ...
    if tf not in TF_RANK:
        return (MtfBreak.UNKNOWN,
                f"ТФ {tf} не из списка стр. 17 — старшинство не определено", None)
    if TF_RANK[tf] == 0:
        ...
        return (MtfBreak.NOT_APPLICABLE,
                ...
                "со второго касания уровень не торгуется", None)
    ...
    for pp in y_read.all_perepriors:
        ...
        return (MtfBreak.CONFIRMED,
                ...
                f"{'ТЕСТ БЫЛ — ' + why if tested else 'теста ещё не было'}", pp)
    return (MtfBreak.ABSENT,
            f"слома на {younger} у этого уровня после касания не было", None)
