...
def trade_source(ex: Exchange, sym: str, report: RunReport,
    ...
    Держится это в памяти одновременно: `Bar` весит 244 байта (замер tracemalloc
    2026-08-21, см. `config.py`; прежнее число 121 отозвано тогда же, копия здесь снята
    ревизией 2026-09-01), и на вселенную расчётно выходит порядка двух гигабайт;
    наблюдаемый расход службы мерился при прежнем числе и подлежит пересмотру.
