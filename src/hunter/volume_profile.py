...
PROFILE_ROW_PRICE_PCT = 0.07
"""Доля цены на строку профиля (0.07% = 0.0007 от цены).

Источник: покадровый разбор видео автора (2026-08-10, докстрока `RowSize.TICKS`) —
на экране автора постоянна ЦЕНА строки ≈0.07%, а не их число. Режим TradingView
Ticks Per Row задаёт высоту строки в тиках; число строк выводится из диапазона.

⚠ Цена 0.07% снята с ОДНОГО экрана корпуса (видимые ~56 строк при цене ~1.36 USD).
Второй проход по видео для подтверждения не сделан — открытый вопрос №2 в
.claude/rules/open-method-questions.md. Если референт изменится, поменяется это число.

⚠⚠ ЧЕГО СТОИТ ЗАМОРОЗКА: при боевом `TV_ROWS = 100` (режим Number of Rows) высота
строки гуляла от 0.0136% до 0.2013% цены — В 15 РАЗ между окнами (замер 2026-08-24,
23 символа, 147 окон, отпечаток в докстроке `TV_ROWS`, удалённой константы).
Смена на постоянную цену строки снимает эту зависимость входа от глубины загрузки.

Откат: вернуть `TV_ROWS = 100` и в `levels.build_level` вызвать `build_tv(rows=100)`.

  ⚠⚠ РЕШЕНИЕ О РЕЖИМЕ ЗАФИКСИРОВАНО, И ЦЕНА ЕГО ШИРЕ ЦЕН ПОК (аудит 2026-08-25).
  Режим сетки определяет не только сдвиг ПОК (медиана 0.0136%, макс 0.1819% на BTC,
  27 общих уровней), но и СОСТАВ карты: на одном правом крае BTC ticks=33 уровня
  против rows=35 — 14 несимметричных ключей. Механизм: отказ «ПОК по сетке TV
  неоднозначен» зависит от сетки, на которой считаются объёмы строк. То есть выбор
  режима есть выбор того, КАКИЕ УРОВНИ СУЩЕСТВУЮТ; смена режима отныне требует той
  же церемонии, что смена горизонта, — две карточки до/после и число цены.
"""

# Совместимость: старый символ для кода, который мог импортировать (нету в проекте, но на случай)
TV_ROWS = 100
"""DEPRECATED: замороженное число строк (режим Number of Rows).
Не используется в боевом пути — оставлен только для совместимости при откате.
См. PROFILE_ROW_PRICE_PCT за текущим параметром (режим Ticks Per Row).
"""


def _ticks_per_row_from_price_pct(
    bottom: Decimal,
    top: Decimal,
    tick_size: Decimal,
    row_price_pct: float = PROFILE_ROW_PRICE_PCT,
) -> int:
    """Вычисляет ticks_per_row для режима Ticks Per Row из процента цены.

    Высота строки в цене = средняя_цена * row_price_pct / 100.
    ticks_per_row = max(1, round(row_height / tick_size)).
    """
    mid_price = (bottom + top) / 2
    row_height = mid_price * Decimal(str(row_price_pct / 100))
    tpr = int((row_height / tick_size).to_integral_value(rounding="ROUND_HALF_UP"))
    return max(1, tpr)
...
class RowSize(StrEnum):
    ...
    ⚠ Чего замер НЕ говорит: он не называет самого числа. «~0.07%» снято с ОДНОГО экрана
    в корпусе, и прежде чем ставить его боевым, нужен второй проход по видео. Пока этого
    нет, переключения не делается — открытый вопрос №2.
    """
