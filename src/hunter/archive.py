...
кэш: свернуть 15м обратно в 5м нельзя. Первая редакция кэша (в зонде, удалённом
19.08) была
на 900_000, и переход сюда потребовал перекачки 3.6 ГБ.
"""


VENUE_MARK = CACHE_DIR / ".venue"
"""Какой площадкой собран кэш. Файл в одну строку.

⚠ ЗАЧЕМ ОН НУЖЕН, И ЭТО РЕГРЕССИЯ 2026-08-11. Имя файла кэша содержит всё, от чего
зависит содержимое, — сутки, корзину, шаг цены, схему бинирования, — но НЕ ПЛОЩАДКУ,
потому что площадка была одна и стояла в коде константой. Как только `venue` стал
ключом конфигурации, инвариант сломался: у спота и USDⓈ-M ОДИНАКОВЫЕ идентификаторы
рынков (`BTC/USDT` и `BTC/USDT:USDT` оба дают `BTCUSDT` — проверено вызовом). Смена
площадки дописывала бы сделки другого рынка в те же файлы, и профиль объёма считался бы
по смеси двух рынков без единого признака.

Полная раскладка по площадкам — этап 4; она требует переноса накопленных суток, а это
данные владельца. Здесь стоит дешёвая замена: метка сверяется при открытии кэша, и
расхождение ОСТАНАВЛИВАЕТ прогон вместо того, чтобы испортить данные.
"""


def check_venue(venue: str) -> str | None:
    """Сверить площадку с меткой кэша. Не `None` — причина, по которой работать нельзя.

    Пустой кэш метится молча: первая площадка, собравшая данные, и становится хозяином
    каталога. Дальше любое расхождение — отказ с названной причиной.
    """
    if not CACHE_DIR.exists():
        return None
    try:
        if VENUE_MARK.exists():
            was = VENUE_MARK.read_text(encoding="utf-8").strip()
            if was and was != venue:
                return (f"кэш сделок в {CACHE_DIR} собран площадкой {was!r}, "
                        f"а запрошена {venue!r}: идентификаторы рынков у площадок Binance "
                        f"совпадают, поэтому дозапись смешала бы разные рынки. "
                        f"Перенесите или удалите каталог, либо верните прежнюю площадку")
            return None
        # Метки нет. Если суточные файлы уже есть — кэш собран ДО введения метки, то есть
        # заведомо площадкой по умолчанию; иначе каталог пуст и метится текущей.
        legacy = next(CACHE_DIR.glob("*.parquet"), None)
        owner = "binanceusdm" if legacy is not None else venue
        VENUE_MARK.write_text(owner + "\n", encoding="utf-8")
        if owner != venue:
            return (f"кэш сделок в {CACHE_DIR} собран до введения метки площадки и "
                    f"принадлежит {owner!r}, а запрошена {venue!r}")
        return None
    except OSError as e:
        log.degraded("метку площадки кэша прочитать не удалось",
...
def orphaned_days(market_id: str, tick: Decimal) -> tuple[int, str]:
    """Сколько суток кэша стали НЕДОСТИЖИМЫ после смены шага цены или схемы.

    ⚠ ЗАЧЕМ, И ПОЧЕМУ ЛЕЧЕНИЕ ИМЕННО ТАКОЕ. Шаг цены стоит в имени файла НЕ по недосмотру:
    в кэш кладутся номера бинов, посчитанные по этому шагу, и файл, снятый при другом
    шаге, этим суткам просто не годится — `bin_price` умножил бы старые номера на новый
    шаг. Убрать шаг из имени значило бы вернуть дефект, закрытый 2026-08-04.

    Настоящий дефект другой: Binance меняет `PRICE_FILTER` штатно, и в этот момент
    накопленные сутки перестают находиться МОЛЧА. Сбор честно объявляет «в кэше ноль» и
    качает всё заново, а на диске рядом лежат гигабайты, про которые никто ничего не
    сказал. Это ровно та форма, что в реестре CLAUDE.md: правдоподобное объяснение
    («данных нет») вместо названной причины («данные есть, но по другому шагу»).

    Возвращает число осиротевших суток и человекочитаемую причину — вызывающий обязан
    напечатать это, а не проглотить. Удалять их код не имеет права: это данные владельца.
    """
    want = f"-{CACHE_BUCKET_MS}-{_tick_tag(tick)}-{CACHE_LAYOUT}"
    other: dict[str, int] = {}
    # ⚠ `CACHE_STEM` сопоставляется с именем ПОСЛЕ отрезания хвоста — это его контракт,
    # и он же однажды уже дал ноль при любом состоянии кэша (см. докстроку шаблона).
    # Здесь хвост заранее неизвестен, поэтому имя разбирается своим шаблоном целиком.
    whole = re.compile(
        rf"^{re.escape(market_id)}-(?P<day>\d{{4}}-\d{{2}}-\d{{2}})(?P<tail>-.+)$"
    )
    for p in CACHE_DIR.glob(f"{market_id}-*.parquet"):
        if p.stem.endswith(want) or "-part" in p.stem:
            continue
        m = whole.match(p.stem)
        if m is None:
            continue
        tail = m["tail"]
        other[tail] = other.get(tail, 0) + 1
    if not other:
        return 0, ""
    parts = ", ".join(f"{tail.lstrip('-')}: {n}" for tail, n in sorted(other.items()))
    return sum(other.values()), (
        f"{market_id}: суток кэша не подходят нынешнему ключу «{want.lstrip('-')}» "
        f"и потому не читаются — {parts}"
    )
...
class WindowSource:
    ...
    def __init__(
        ...
        `run.persist_source` (до 2026-08-18 — `persist_archive`), и на этом стоит
        герметичность повтора.
    ...
    def window(self, from_ms: int, to_ms: int) -> TradeHistogram | NotReady:
        ...
        if missing and self.live is not None:
            ...
            # зонд probe_live_overlap_2026-08-09 (удалён 19.08) — на подстроенном
