...
def retest_after(
    ...
    *,
    ...
    Ретестом считается касание линии уровня — бар, чей диапазон её накрывает, ЛИБО пара
    соседних баров, между которыми цена перешла через линию не коснувшись.

    ⚠⚠ ВТОРОЙ СЛУЧАЙ ДОБАВЛЕН 2026-08-22, И ЭТО БЫЛА ДЫРА В РАСЧЁТЕ. Прежде требовалось
    ТОЛЬКО накрытие (`low <= level <= high`), и при разрыве цены ретест терялся: бар
    слева целиком ниже линии, бар справа целиком выше, линию не накрыл никто. Соседняя
    функция того же файла — `first_breach` — от разрывов защищена (`_beyond` смотрит один
    бар), то есть две функции отвечали на ОДИН человеческий вопрос «цена пришла к
    уровню» по РАЗНЫМ правилам. Это ровно запрещённая конструкция «две сущности под
    одним именем»: расхождение молчит, пока не встретится разрыв.

    Цена дефекта на нашей вселенной МАЛА, и это названо честно (замер 2026-08-22, свежие
    REST-бары, 998 пар соседних баров на ряд): у BTC, ETH, SOL разрывов ЦЕНЫ **ноль** на
    5м/15м/1ч; у обязательных символов владельца — XAUT 5м 1.002% пар, 15м 0.501%, 1ч
    0.100%; PAXG 5м 0.301%. Но САМИ разрывы крошечные: максимум 0.008% цены. То есть
    сегодня событие почти не встречается — правка ставится на ПРИБОР, а не на
    случившийся ущерб, и переживёт расширение вселенной на инструменты с настоящими
    разрывами.

    ⚠ ДЫРА В РЯДУ РАЗРЫВОМ ЦЕНЫ НЕ СЧИТАЕТСЯ. Если между барами пропущены бары
    (`steps_between != 1`), о движении цены в промежутке неизвестно ничего, и объявлять
    там ретест значило бы выдумывать. Поэтому нужен `timeframe` — та же защита и тем же
    способом, что в `first_breach` (строка 233).
    """
    stop = len(bars) if to_index is None else min(to_index, len(bars))
    for i in range(max(from_index, 0), stop):
        ...
        if i > 0:
            prev = bars[i - 1]
            if steps_between(prev, bar, timeframe) != 1:
                continue
            if prev.high < level < bar.low or bar.high < level < prev.low:
                return i
    return None
