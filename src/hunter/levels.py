...
⚠ ЧИСЛО НАШЕ, А НЕ КУРСА, и вот на чём оно стоит. Курс определяет флэт как «движение цены
в горизонтальном ценовом диапазоне» (стр. 18) и добавляет «Цена ходит от одной границы к
другой», но высоты не ограничивает нигде; стр. 30 наоборот называет «накопление очень
большое ТФ 1Д-1Н-1М» штатным случаем. Значит нужен признак, отличающий очень большое
накопление от ВСЕЙ ИСТОРИИ АКТИВА, и он взят замером, а не выдуман.

ЧТО ЗАМЕРЕНО (2026-08-20, весь кэш баров: 25 символов, 4280 закрытых структур).
Высота коробки, делённая на ATR(14) на первом баре структуры, оказалась ПОЧТИ ИНВАРИАНТНОЙ
по ТФ — то есть это свойство метода, а не таймфрейма:

    ТФ    структур   медиана   p90    p99     макс
    5м       913       6.9     15.4   39.2     72.6
    15м     1043       6.5     12.2   28.2     76.6
    1ч      1190       6.3     12.0   30.0    123.5
    4ч       872       6.2     11.2   31.9     55.3
    1Д       187       6.0     13.1   28.0     99.0
    1Н        75       6.6     29.0  2493.8  2493.8

База ростом в шесть своих ATR — это метод. База ростом в 2494 ATR — это не база.

СКОЛЬКО СТОИТ ОТСЕЧКА (структур выше порога из тех же 4280):

    порог    5м  15м  1ч  4ч  1Д  1Н   всего
      40      7    5   5   2   1   6     26
      60      1    3   1   0   1   6     12
     100      0    0   1   0   0   5      6   ← стоит
     150      0    0   0   0   0   3      3
     300      0    0   0   0   0   2      2

Сотня выбрана потому, что ниже неё не выбрасывается НИЧЕГО на 5м-4ч и 1Д, а выше остаются
ровно шесть структур из 4280 (0.14%), пять из них недельные. Это разрыв в распределении, а
не круглое число: максимум по всем ТФ кроме 1Н — 123.5, а следующее значение у 1Н — 2493.8.

⚠ ЧЕМ ЭТО НЕ ЯВЛЯЕТСЯ. Это не «фильтр плохих сигналов» — такие запрещены правилом
2026-08-17 («порог на выходе не лечит расчёт на входе»). Это проверка ОПРЕДЕЛЕНИЯ: объект,
не отвечающий определению флэта, флэтом не считается. Отказ ИМЕНУЕТСЯ и попадает в
`unbuilt` — молча не исчезает ничего.

ЧТО ЭТО НЕ ЧИНИТ. Почему на 1Н вообще возникают такие коробки — не разобрано. Механизм
частично понятен (коробку в сотни ATR цена не покидает, двух тел за границей не бывает,
структура не закрывается), но полного объяснения нет, и пункт остаётся открытым в
докстроке `accumulation`.

ПОВОД. Найдено чтением сообщений бота глазами на живых данных: на BTC при цене около
69 000 уведомление предлагало «ПРОДАЖА, лимитка 111 066, зона 106 905–119 222 (1Н), стоп
129 995, риск 17.0%, цель 26 846». Это недельная «структура» размером во всю историю.
"""


def _atr14(bars: list[Bar], upto: int) -> float:
    """ATR(14) на баре `upto`. Своя, а не из `indicators`: тот модуль строит ПОЛНЫЙ ряд,
    а здесь нужна одна точка, и полный ряд на каждую структуру стоил бы квадрата."""
    lo = max(0, upto - 13)
    trs: list[float] = []
...
def _base_too_tall(acc: Accumulation, bars: list[Bar]) -> str | None:
    ...
    ref = _atr14(bars, acc.first_index)
    if ref <= 0:
        return None
    ratio = height / ref
