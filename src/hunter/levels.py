...
def resolve_carried(side: str, price: float, timeframe: str,
    ...
    ⚠ ЦЕНА ПРАВКИ ЗАМЕРЕНА НА ЖИВЫХ ДАННЫХ, а не оценена. Отпечаток: 570 активных
    уровней леджера из 591 (21 отсеян нехваткой свежих баров), свежие REST-бары
    `binanceusdm`, 2026-08-23. Оба правила прогнаны на одних и тех же барах:
    ...
    ⚠⚠ И там же вскрылось, что ветка `flipped` прежнего правила была ПРАКТИЧЕСКИ МЁРТВОЙ:
    на всей выборке она не сработала НИ РАЗУ (нет ни одной строки «БЫЛО flipped»). Тела
    за коробкой структуры почти не случаются — коробка для этого слишком широка. То есть
    прежнее правило сводилось к «отработка или ничего», и отработка была переохотной.
    Это ровно форма Д-1: ветка, которая не может сработать, и счётчик, который не может
    оказаться неверным.
