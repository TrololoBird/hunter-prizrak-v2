...
def build_level(
    ...
    # ⚠ ПОК ОБЯЗАН ЛЕЖАТЬ ВНУТРИ БАЗЫ. Правка 2026-08-11, повод — вопрос владельца:
    # как ПОК вообще может оказаться вне базы, если базу мы определяем первой.
    #
    # Курс, стр. 21, дословно: «ПОК = POC = Point Of Control - максимальный уровень
    # ликвидности (проторговки, где было больше всего продаж/покупок) какой-то зоны на
    # графике. В разборах/анализе используем - ПОК конкретных накоплений, т.е.
    # максимальный уровень проторговки базы». То есть ПОК — максимум проторговки БАЗЫ,
    # а не любой цены, попавшей в окно профиля.
    #
    # Почему такое вообще возможно у нас, а у автора нет. У него ОДНА сущность: он
    # выделяет коробку по свечам, профиль строится ровно по ней, стоп ставится за её
    # край. У нас с 2026-08-10 их ДВЕ: линии границ по точкам свингов (`acc.*.edge`,
    # ими детектируется выход) и диапазон профиля по крайним ценам баров (стр. 26:
    # «важно захватить все свечи структуры»). Второй шире первого в 3.156 раза по
    # медиане — и максимум объёма может прийтись на прокол ЗА границей.
    #
    # Замер 2026-08-11 на карте из 856 уровней: у 185 (21.6%) ПОК лежал вне
    # `boundary_lo…boundary_hi`. Цена этого не косметическая: вход брался от ПОК, стоп —
    # от границы, то есть две цены из разных систем координат, и риск раздувался.
    # Внешние определения POC («the single most-traded price within a given session,
    # range or distribution» — TradingView, CQG, Trader Dale и ещё семь источников)
    # разрешают ПОК только ВНУТРИ диапазона профиля, но про соотношение с линиями
    # границ молчат: у них такого различения просто нет.
    #
    # Поэтому здесь ОТКАЗ (§4.3), а не поправка цены: подвинуть ПОК внутрь значило бы
    # выдумать уровень, которого профиль не показывал.
    lo, hi = Decimal(str(acc.lower.edge)), Decimal(str(acc.upper.edge))
    if not lo <= profile.poc_price <= hi:
        return NotReady(
            reason=f"{symbol} {acc.timeframe}: ПОК {profile.poc_price} вне базы "
                   f"[{lo}, {hi}] — максимум объёма пришёлся на прокол, а не на "
                   f"проторговку базы (стр. 21)"
        )
