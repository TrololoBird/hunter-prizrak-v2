...
def _one_level_per_range(
    ...
    Взято второе: охват автора НЕ ПАДАЕТ, а девять дублей уходят. Старший ТФ выигрывает
    не по старшинству как таковому, а по стр. 23–24 — уровень принадлежит ТФ, где структуру
    видно чётко, и работает на младших; замер это же и подтвердил (промах втрое точнее,
    чем у правила «узкая зона»).

    ⚠ СТОРОНА ОСТАВЛЕНА В КРИТЕРИИ. Без неё уходит ещё четыре уровня, но вместе с ними
    теряется одна линия автора: лонг-уровень и шорт-уровень на одной цене — разные
    объекты (стр. 43 переворачивает пробитый уровень).

    Критерии геометрические, порогов не вводится:
      * ЗОНЫ ПЕРЕСЕКАЮТСЯ — значит это один и тот же ценовой уровень;
    ...
    for lvl in sorted(built, key=lambda x: -order.get(x.timeframe, 0)):
        senior = next(
            (k for k in keep
             ...
             and not (lvl.zone_hi < k.zone_lo or k.zone_hi < lvl.zone_lo)),
            None)
        if senior is None:
            keep.append(lvl)
            continue
        unbuilt.append(Unbuilt(
            ...
            reason=(f"эта цена уже есть уровнем на {senior.timeframe} "
                    f"(ПОК {senior.price:.8g}, зоны пересекаются): стр. 23 — уровень "
