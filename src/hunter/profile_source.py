...
class CandleWindows:
    ...
    def window(self, from_ms: int, to_ms: int) -> TradeHistogram | NotReady:
        ...
        for v in vols_kept.tolist():
            qty_seen += v
        h.qty_seen = qty_seen
        # ⚠⚠ РАВНОМЕРНАЯ РАСКЛАДКА — ЭТО ПРИБОР АВТОРА, А НЕ НАШЕ ПРИБЛИЖЕНИЕ. Проверено
        # 2026-08-22 по внешнему источнику после разбора досье `Documents/Prizzrak`:
        # TradingView кладёт объём бара РОВНО ПОРОВНУ по всем строкам, которые накрывает
        # его диапазон, — «A bar spanning 5 rows adds one fifth of its volume to each»
        # (сводка справки и разборов TV, прочитано 2026-08-22). Наша формула — та же.
        #
        ...
        # РАЗМЕТКОЙ АВТОРА, который смотрит в TradingView. Прибор автора равномерен;
        # «улучшив» раскладку, мы стали бы точнее против тиков и ДАЛЬШЕ от него.
