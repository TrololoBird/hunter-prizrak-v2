...
def live_unique(
    ...
    +1270 (зонд свидетельство E-flipped-hidden-2026-08-19 (удалено 19.08)probe_flipped.py).
    ...
    return alive, _dedupe(alive, price), off_struct, _dedupe(flipped, price)
...
def board_digest(levels: tuple[LevelRow, ...], prices: dict[str, float],
    ...
    lines.append("Порядок: по тренду выше встречных (стр. 47) · вход сейчас выше "
                 "ожидания слома (стр. 31) · близкое выше далёкого и выше уже вошедшего "
                 "в зону (стр. 44) · старший ТФ выше младшего (стр. 48) · РР (стр. 9).")
    return "\n".join(lines)
