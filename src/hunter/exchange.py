...
# ⚠ Старый СКАЛЯР `WS_SILENCE_S` (порог молчания потока БАРОВ) удалён 2026-08-05
# вместе с тем потоком. ⚠⚠ Ревизия 2026-09-01: имя ПЕРЕИСПОЛЬЗОВАНО — ниже в этом же
# файле живёт СЛОВАРЬ `WS_SILENCE_S` порогов молчания WS (включая "ohlcv"), так что
# фраза «удалён» про имя стала ложным указателем; удалён именно старый скаляр.
...
class Exchange:
    ...
    async def _watch_step(
        ...
        Числа и их замеры — в `WS_TRADES_SILENCE_S` (старый скалярный порог баров
        удалён вместе с ними; действующие пороги — словарь `WS_SILENCE_S` ниже).
