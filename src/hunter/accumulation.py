...
|p1 − p2| первой пары стороны к ATR(14) на первом баре структуры:
    ...
    5м     1842     1.46     5.19    19.6     67.1       62 / 17 /  1 /   0
    15м    2070     1.29     4.46    11.2     41.5       28 /  2 /  0 /   0
    1ч     2370     1.25     4.16    11.1     38.8       29 /  7 /  0 /   0
    4ч     1746     1.27     4.34    13.0     44.1       25 /  9 /  0 /   0
    1Д      374     1.35     4.45    15.2     76.3        8 /  3 /  1 /   0
    1Н      150     1.30    12.46   527.4   1317.8       16 / 10 /  9 /   5
...
ПЯТЬДЕСЯТ выбрано по разрыву, а не округлением: выше пятидесяти на всех ТФ кроме 1Н
остаются ДВЕ стороны из 8402 (одна на 5м, одна на 1Д), а на 1Н — девять из 150. Ниже
пятидесяти распределения всех шести ТФ неразличимы.
...
def atr14(bars: list[Bar], upto: int) -> float:
    """ATR(14) на баре `upto`. Единица «одна свеча этого ТФ».

    ⚠ Одна на всех потребителей: её же зовёт `levels._base_too_tall`. Вторая копия
    средней истинной разницы разошлась бы с первой на первой правке — тот же довод, что
    у `structure_bars`.
    """
    lo = max(0, upto - 13)
    trs: list[float] = []
    for i in range(lo, min(upto + 1, len(bars))):
