...
Разбор — в докстроке `MAX_PAIR_ATR`; коротко: нижнюю границу недельной структуры SOL
...
Верная мерка нашлась, когда знаменателем стала величина, дефектом НЕ затронутая, — ATR
своего ТФ на баре первой точки. По ней тот же разброс равен 247 ATR при медиане 1.3 по
всем шести ТФ. Правило и замер — в докстроке `MAX_PAIR_ATR`.

Порога высоты как ПЕРВОЙ линии не введено и здесь: курс его не даёт, а стр. 30 называет
«накопление очень большое ТФ 1Д-1Н-1М» штатным случаем. Корень чинится повтором точек
(стр. 13), а проверка высоты осталась второй линией в `levels.MAX_BASE_ATR`.
"""
...
MAX_PAIR_ATR = 50.0
"""Насколько далеко друг от друга МОГУТ стоять две точки ОДНОЙ границы, в ATR своего ТФ.
...
Словами курс это говорит, числом — нет, поэтому число ЗАМЕРЕНО, и замер предъявляется.

ЗАМЕР 2026-08-20, весь кэш баров (25 символов, 3000 баров на ТФ), отношение
|p1 − p2| первой пары стороны к ATR(14) на первом баре структуры:

    ТФ    сторон   медиана   p90     p99     макс     выше 10 / 20 / 50 / 100
    5м     1906     1.44     5.06    18.7    41.2       59 / 15 /  0 /   0
    15м    2162     1.30     4.47    11.3    41.5       30 /  4 /  0 /   0
    1ч     2458     1.24     4.16    12.1    38.8       32 /  9 /  0 /   0
    4ч     1822     1.27     4.41    13.4    44.1       28 / 10 /  0 /   0
    1Д      368     1.35     4.38    10.6    20.3        6 /  1 /  0 /   0
    1Н      144     1.30     6.52    43.0    52.9        9 /  2 /  1 /   0

⚠⚠ ТАБЛИЦА ПЕРЕСНЯТА 2026-08-20 ПОСЛЕ ИСПРАВЛЕНИЯ САМОГО ATR. До того `atr14` считал
простое среднее последних 14 значений истинного диапазона, то есть НЕ ATR (разбор — в его
докстроке). Прежние числа этой таблицы (1Н: p99 527.4, максимум 1317.8, девять сторон
выше 50) сняты на той величине и ОТОЗВАНЫ; они относились к состоянию ДО правила повтора.
Нынешние сняты настоящим ATR Уайлдера и уже с действующим правилом

МЕДИАНА 1.25–1.46 НА КАЖДОМ ТФ — то есть «повтор» это свойство метода, а не таймфрейма:
две точки одной границы обычно стоят в полутора ATR друг от друга. Хвост же есть только у
1Н: p99 там 527 против 11–20 у остальных.

ПЯТЬДЕСЯТ оставлено и после перемера: выше него по всему кэшу остаётся РОВНО ОДНА сторона
из 8860 (на 1Н, 52.9). Медианы 1.24–1.44 и p90 4.16–6.52 совпадают у всех шести ТФ, то
есть «повтор» остаётся свойством метода, а не таймфрейма.
...
⚠ ЧИСЛО НАШЕ. Курс порога не называет; названо им СВОЙСТВО (повтор), а мера свойства
взята замером на инварианте, который сам проверен по шести ТФ.
"""


def atr14(bars: list[Bar], upto: int, window: int = 14) -> float:
    """ATR Уайлдера на баре `upto`. Единица «одна свеча этого ТФ».

    ⚠⚠ ИСПРАВЛЕНО 2026-08-20: ДО ЭТОГО ДНЯ ЭТО БЫЛ НЕ ATR. Стояло простое среднее
    последних 14 значений истинного диапазона, а ATR Уайлдера — сглаживание по ВСЕЙ
    истории: затравка средним первых `window` значений, дальше
    `atr[i] = (atr[i-1]*(window-1) + tr[i]) / window`.

    Найдено сверкой с чужими реализациями, которые до того дня остались непрочитанными
    (владелец спросил прямо, доведён ли разбор). Определение совпадает у двух
    независимых источников корпуса:
      bukosabino/ta (5145 звёзд), ta/volatility.py, класс AverageTrueRange — цикл
        `atr[i] = (atr[i-1] * (window-1) + true_range[i]) / window` после затравки
        `atr[window-1] = true_range[0:window].mean()`;
      TA-Lib (12194 звезды) — канонический ATR той же формулы.

    ЦЕНА ОШИБКИ ЗАМЕРЕНА (наша прежняя величина, делённая на ATR Уайлдера; 8 символов,
    1500 баров на ТФ):
        ТФ    медиана   p10     p90    расходятся больше чем на 25%
        5м     0.985   0.850   1.131        6.5%
        1ч     0.979   0.841   1.134        8.1%
        4ч     0.976   0.831   1.118        7.8%
        1Д     0.955   0.808   1.128       10.5%
        1Н     0.842   0.629   1.186       47.8%
    То есть на недельном ТФ — там, где живёт неразобранный дефект абсурдных коробок, —
    прежняя величина была систематически на 16% меньше настоящего ATR, и почти половина
    точек расходилась больше чем на четверть.

    ⚠ Обе отсечки, стоявшие на прежней величине (`MAX_PAIR_ATR` и `levels.MAX_BASE_ATR`),
    ПЕРЕСНЯТЫ после этой правки — калибровка на величине с неверным именем не переносится.

    ⚠⚠ §10.2 ГЛАСИТ «Индикаторы: не писать свои», И ЭТО ПРАВИЛО ЗДЕСЬ НЕ НАРУШЕНО —
    но доказано это замером, а не объявлено. Своя функция стоит по архитектурной причине:
    `indicators.py` отдаёт ВЫРАЖЕНИЯ polars, а этому модулю нужно ОДНО ЧИСЛО на заданном
    индексе внутри чистого прохода по `list[Bar]`; тянуть сюда polars значило бы сломать
    §10.3 (чистый модуль).

    Равенство библиотечной реализации ПРОВЕРЕНО 2026-08-20: наша величина против
    `polars_talib.atr(timeperiod=14)` на 371 точке (5 ТФ × 6 символов × каждый 37-й бар из
    последних 800) — наибольшее относительное расхождение 4.241e-07, худший случай
    ANKR 1Н бар 200: наш 0.00656009348 против 0.006560096262. Это шум накопления float,
    а не разные формулы.

    ⚠ Отдельно: `indicators.py` с 2026-08-06 нёс запись «Вернуть [обёртку над plta.atr] —
    три строки. Пока нет порога, выраженного в ATR, возвращать нечего». Условие ТЕПЕРЬ
    ВЫПОЛНЕНО: порогов в ATR стало два (`MAX_PAIR_ATR` здесь и `levels.MAX_BASE_ATR`).
    Обёртка не заведена именно из-за формы ответа — выражение против скаляра; замер выше
    заменяет её тем, ради чего она была нужна.

    Одна на всех потребителей: её же зовёт `levels._base_too_tall`.
    """
    if upto <= 0 or not bars:
        return 0.0
    end = min(upto, len(bars) - 1)
    tr: list[float] = []
    for i in range(0, end + 1):
        pc = bars[i - 1].close if i else bars[i].open
        tr.append(max(bars[i].high - bars[i].low,
                      abs(bars[i].high - pc), abs(bars[i].low - pc)))
    if len(tr) < window:
        return sum(tr) / len(tr) if tr else 0.0
    val = sum(tr[:window]) / window
    for i in range(window, len(tr)):
        val = (val * (window - 1) + tr[i]) / window
    return val
...
def detect(
    ...
    *,
    ...
    def repeats(price: float, first: float, first_index: int) -> bool:
        ...
        что у двух точек ATR различается в полсотни раз, уже говорит, что они из разных
        режимов и одной границей быть не могут.
        Порог и замер под ним — в докстроке `MAX_PAIR_ATR`. ATR ноль (ряд короче
        одного бара либо свечи без размаха) означает «мерить нечем», и тогда пара
        принимается: молча отбрасывать по неизмеримому признаку нельзя (§4.3).
        """
        ref = atr14(bars, first_index)
        return ref <= 0 or abs(price - first) <= MAX_PAIR_ATR * ref
