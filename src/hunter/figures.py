...
def channel_early_pp(channel: Channel, pps: Sequence[Pereprior]) -> Pereprior | NotReady:
    ...
    if not fits:
        # ⚠ ЗДЕСЬ СТОЯЛО «вход по стр. 56 ОСТАЁТСЯ только от ближайшего уровня»
        # (правка 2026-08-26, Ф2). Слово «остаётся» УТВЕРЖДАЛО доступность второго входа,
        # которой этот модуль знать не может: уровней он не видит, а стр. 56 разрешает
        # вход от ближайшего уровня только «если он есть». При фигуре без живого уровня
        # своей стороны строка обещала владельцу вход, которого нет ни одного. Есть ли
        # уровень — отвечает `card._entries` на строке САМОЙ фигуры, где счёт живых
        # уровней стороны и печатается; здесь называется только то, что известно тут.
        return NotReady(
            reason=f"раннего ПП стороны {want.value} внутри фигуры нет — "
                   "вход стр. 56 «на подтверждение раннего ПереПриора» недоступен"
        )
