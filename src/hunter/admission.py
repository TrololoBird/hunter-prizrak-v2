...
# ATR — не индикатор §2.9, а единица измерения порогов по §4.1.
...
def seed_floor() -> int:
    ...
    Это §2.9 (`USED_BY_2_9`, строжайшее — ema200 с 200) плюс `atr14` — не индикатор,
    а единица измерения порогов §4.1 (его 15 баров поглощаются двумястами).
    ...
    return max(strictest_requirement(), REQUIRED_BARS["atr14"])
