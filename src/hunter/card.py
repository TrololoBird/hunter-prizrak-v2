...
def render(d: engine.SymbolDecision, series: dict[str, list[Bar]]) -> str:
    ...
    for dec in d.decisions:
        ...
        # ⚠ ПРАВИЛО СТР. 44 БОЛЬШЕ НЕ ПЕЧАТАЕТСЯ ЦЕНОЙ (правка 2026-08-24). Его
        # `watch_price` — сама ТВХ, поэтому строка выходила «безубыток 75.945 … пробой
        # уровня и ретест — следить за 75.945»: цена наблюдения РАВНА цене открытия, то
        # есть условие выполнено в момент входа всегда. Владелец читает карточку и не
        # может знать, что за одинаковыми числами стоят разные события; напечатанное
        # число там ничего не значит, а выглядит как порог.
        #
        # Условие стр. 44 ценой и НЕ выражается: это "цена прошла уровень и вернулась на
        # него с другой стороны", то есть последовательность, а не порог. Печатается
        # словами. Подставлять сюда какое-нибудь другое число значило бы сочинить порог,
        # которого курс не даёт (§4.3).
        rules = "; ".join(
            BREAKEVEN_LABEL[b.trigger.value]
            + ("" if b.trigger is geometry.BreakevenTrigger.LEVEL_BREACH_RETEST
               else f" — следить за {_num(b.watch_price)}")
        ...
        if s.targets:
            ...
            why = s.no_target_reason or "причина не посчитана — сделка старой схемы"
            out.append(f"        целей нет — {why}")
        if s.add_ons:
