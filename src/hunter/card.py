...
STOP_BASIS_LABEL = {
    "anchor": "спрятан за прокол, стоповый объём или лой/хай ТФ−1 (стр. 18)",
    "margin": "запас за структуру (стр. 33)",
}
"""Запасная подпись основания стопа: печатается, только когда конкретный предмет назвать
нечем. Обычный путь называет ОДИН предмет — тот, который сработал (см. `compose_text`).

⚠ Прежняя подпись `anchor` гласила "за стоповым объёмом либо проколом" (наши слова, не
цитата) и была НЕПОЛНОЙ: стр. 18 называет ТРИ предмета, и третий — лой того же ТФ или
ТФ−1 — заведён 2026-08-09, а в подпись не попал. Исправлено 2026-08-23."""
...
"""За что курс велит прятать стоп — три предмета из одной фразы стр. 18: «Если в диапазоне
2-5% от границы есть стоповый объем – база мелкого ТФ - или Лой того же ТФ или ТФ-1 -
идеально стоп прятать за них». Печатаются с 2026-08-09: до этого оператор видел цену стопа
и не мог сказать, откуда она взялась (§4.3).

⚠ СЛОВА «ЯКОРЬ» ЗДЕСЬ БОЛЬШЕ НЕТ (2026-08-23). Его нет ни в курсе, ни в корпусе, и
печатать его владельцу как термин метода было выдумкой; наружу идёт формулировка курса
«прятать за». Внутренние имена (`stop_anchor`, `StopAnchorSource`) оставлены как
служебное обобщение трёх фраз и термином метода не притворяются."""
ENTRY_LABEL = {
...
def render(d: engine.SymbolDecision, series: dict[str, list[Bar]]) -> str:
    ...
    for dec in d.decisions:
        ...
        # ⚠ Чем оказался стоп, печатается с 2026-08-09. До этого оператор видел цену
        ...
        #
        # ⚠⚠ СЛОВО «ЯКОРЬ» УБРАНО ИЗ КАРТОЧКИ 2026-08-23 по замечанию владельца, и он
        # прав механически: такого термина НЕТ НИ В КУРСЕ (греп по текстовому слою всех
        # 69 страниц — ноль вхождений), НИ В КОРПУСЕ (там «якорь» встречается четыре раза
        # и всюду про ручки трендовой линии в TradingView, то есть про интерфейс). Слово
        # было нашим обобщением трёх разных фраз стр. 18 и выдавалось владельцу за термин
        # метода. Внутри кода имя остаётся (`stop_anchor`, `StopAnchorSource`) — это
        # служебное обобщение, — а наружу идёт формулировка самого курса: «прятать за».
        #
        # ⚠⚠ И ТОЙ ЖЕ ПРАВКОЙ СНЯТО ПРОТИВОРЕЧИЕ ВНУТРИ СТРОКИ. Прежде основание стопа
        # печаталось подписью «за стоповым объёмом либо проколом (стр. 18)», хотя видов
        # ТРИ: свинг добавлен 2026-08-09 и в подпись не попал. Карточка могла напечатать
        # «стоп … — за стоповым объёмом либо проколом (стр. 18) … якорь: лой/хай своего
        # ТФ», то есть назвать предмет дважды и по-разному. Теперь предмет называется
        # ОДИН раз — тот, который сработал.
        if lvl.stop_anchor_source is not None and s.stop_basis.value == "anchor":
            basis = f"спрятан за {ANCHOR_LABEL[lvl.stop_anchor_source.value]} (стр. 18)"
            if lvl.stop_anchor_narrowed:
                basis += " в сужении (стр. 34)"
        else:
            basis = STOP_BASIS_LABEL[s.stop_basis.value]
