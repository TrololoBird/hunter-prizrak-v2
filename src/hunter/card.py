...
DIV_LABEL = {
    ...
    # семействами, а текст описывает одно.
    # ⚠ ЗДЕСЬ СТОЯЛО «Расширенных нет — им нужен допуск на равенство экстремумов,
    # которого курс не даёт (§0)» — снято 2026-08-23: расширенные подписи стоят прямо
    # НИЖЕ и добавлены 2026-08-09, то есть комментарий опровергался тремя строками
    # собственного словаря. Допуск нашёлся не в курсе, а в классике (Булковски <3%,
    # `factors.EXTENDED_EQUAL_TOL_PCT`), и это записано у самих подписей.
...
ENTRY_LABEL = {
    # ⚠ БЫЛО «лимитки на ПОК и зону (стр. 30)» — исправлено 2026-08-23. Подпись
    # говорила, КУДА ставить ордера, а владелец 2026-08-19 запретил ровно это:
    # «не надо писать где именно ордера ставить, пользователь сам решит на основе
    # зоны и ПОК». Тем приказом из geometry.py удалили `build_entry_orders` и
    # лестницу, а подпись карточки — единственное место, где раскладка предъявлялась
    # владельцу ГЛАЗАМИ, — пережила правку и продолжала её печатать.
    #
    # Вторая причина, независимая от приказа: подпись обещала то, чего расчёт не
    # делал. Лестницы входа в проекте нет с 2026-08-19, ордер один и он на ПОК
    # (`Setup.entry = level.price`), «и на зону» не выставлял никто. Найдено внешним
    # обзором 2026-08-19, находка A-14.
    #
    # Что осталось: НАЗВАНО РАЗРЕШЕНИЕ, а не раскладка. Различие с двумя другими
    # значениями `EntryRule` — именно в разрешении (стр. 31 снимает лимитки, стр. 43
    # переводит вход на ретест), и оно сохранено.
    "limit": "вход лимитным ордером разрешён (стр. 30)",
...
# ⚠ `ENTRY_ORDER_LABEL = {"zone": "на зону", "poc": "на ПОК"}` УДАЛЁН 2026-08-23:
# подписи для ордеров входа, которых нет с 2026-08-19 (`build_entry_orders` и
# `Setup.entry_orders` удалены тем днём по приказу владельца «не надо писать где именно
# ордера ставить»). Ноль вхождений кроме собственного объявления — мёртвая величина.
...
def trim_zeros(s: str) -> str:
    """Убрать хвостовые нули дробной части И осиротевшую точку. ОДНА функция на проект.

    ⚠⚠ ЗАВЕДЕНА 2026-08-23, ПОТОМУ ЧТО КОПИЙ БЫЛО ДВЕ И ОНИ УЖЕ РАЗОШЛИСЬ. В `card._num`
    стояло `.rstrip("0").rstrip(".")`, в `tgbot._fmt_price` — только `.rstrip("0")`, и
    вторая печатала НОЛЬ как `0.`: `f"{0.0:.8f}"` даёт `0.00000000`, снятие нулей
    оставляет одинокую точку. Ровно тот класс «две сущности под одним именем», из-за
    которого в этом проекте полгода жил неверный стоп, — только здесь он виден глазами
    в сообщении бота.

    Целое число без дробной части не трогается: `"100"` остаётся `"100"`, а не
    превращается в `"1"`. Пустая строка после снятия (вход был `"0.000"`) — это ноль,
    и он печатается нулём, а не пустотой.
    """
    if "." not in s:
        return s
...
def _num(x: float | Decimal, digits: int = 8) -> str:
    ...
    ⚠ `Decimal` проходит через `float`, и это НАЗВАННОЕ ограничение, а не недосмотр:
    `format(x.normalize(), "f")` дал бы то же без круга через двоичную дробь, но при
    восьми знаках после запятой обе дороги совпадают (float64 несёт 15-17 значащих
    цифр, а цена с восемью знаками их не исчерпывает). Менять путь ради чистоты значило
    бы сдвинуть ТЕКСТ КАРТОЧКИ без предъявления двух карточек — правку расчёта в этом
    проекте предъявляют результатом, а не рассуждением.
    """
...
def render(d: engine.SymbolDecision, series: dict[str, list[Bar]]) -> str:
    ...
    for dec in d.decisions:
        ...
        out.append(
            ...
            # коробки. Печатается рядом с границами, потому что сетка натянута ровно на
            # них. Разбор — докстрока `Level.row_height`.
        ...
        # ⚠ ПРАВИЛО СТР. 44 БОЛЬШЕ НЕ ПЕЧАТАЕТСЯ ЦЕНОЙ (правка 2026-08-24): его условие
        # ценой не выражается — «пробой уровня и ретест» это последовательность, а не
        # порог, и напечатанная ТВХ выглядела порогом, выполненным в момент входа.
        #
        # ⚠ Ревизией того же дня проверка ушла С ТРИГГЕРА НА ТИП: `watch_price` у такого
        # правила теперь `None` в самой модели (разбор — докстрока
        # `BreakevenRule.watch_price`), и карточка больше не перечисляет триггеры,
        # у которых «на самом деле» нет цены, — бесценовость видна полем.
        rules = "; ".join(
            BREAKEVEN_LABEL[b.trigger.value]
            + ("" if b.watch_price is None else f" — следить за {_num(b.watch_price)}")
            for b in s.breakeven_rules)
        ...
        if s.targets:
            ...
            # ⚠⚠ ПРИЧИНА ОБЯЗАТЕЛЬНА (правка 2026-08-24). Здесь печаталось голое
            # «целей нет» — прибор, который молчал, пока владелец не сказал: «Сделок
            # без цели быть не может. Это означает ошибка в расчетах, геометрии,
            # структурах, поках, объемах или во всем вместе!» Отсутствие цели читалось
            # как свойство рынка, хотя на разобранных сделках 1ч/4ч/1Д причиной был
            # ПУСТОЙ ПУЛ — уровни старших ТФ не построились (покрытие профиля). Раздел
            # ПП строками ниже причины называет с 2026-08-20; уровень отставал. §4.3.
            # ⚠ Запасной текст «причина не посчитана — сделка старой схемы» убран
            # 2026-08-24: `Setup` строится в ОДНОМ месте (`geometry.build_setup`), и
            # `why_empty` там непуст при пустых целях по построению (безусловный else
            # в конце цепочки фильтров). Ветвь печатала состояние, которого не бывает,
            # — читатель заключал, что «сделки старой схемы» доходят до карточки.
            out.append(f"        целей нет — {s.no_target_reason}")
        if s.add_ons:
...
def _pennant(pen: figures.Pennant,
    ...
    ⚠⚠ СТОП ПЕЧАТАЛСЯ БЕЗ ЗАПАСА, ИСПРАВЛЕНО 2026-08-24. Строка была «стоп прячем за
    {stop_anchor} (стр. 57)» и ставила под слово «стоп» РОВНО край структуры: у всех 48
    вымпелов живого прогона стоп стоял на границе. Запаса не накладывал никто (разбор —
    докстрока модуля `figures`), а подпись стояла одна на обе фигуры — «стр. 57», в том
    числе у той, которую сама же карточка называет треугольником стр. 58; именно стр. 58
    и требует «с запасом 1-3%». Теперь печатаются ОБЕ величины со своими страницами:
    цена стопа (стр. 58) и то, за что он спрятан (стр. 57).
    """
    ...
    return (f"{PENNANT_LABEL[pen.borders.value]} {FIGURE_SIDE_LABEL[pen.side.value]}  "
            ...
            f"стоп {_num(pen.stop_price)} — за структуру с запасом "
            f"{geometry.DEFAULT_MARGIN_PCT:.0f}% (стр. 58), "
