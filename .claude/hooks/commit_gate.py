...
OK_PREFIX = "ok"
"""Приставка строки УСПЕХА. Вывод читается инверсией: выбрасывается успех, остаётся всё
остальное — независимо от того, какими словами гейт назвал свой отказ."""

MAX_LINES = 60
"""Потолок печати. ⚠ Урезание НАЗЫВАЕТСЯ числом, а не делается молча: молчаливое
усечение читается как "это всё, что было" (§4.3 — молчаливый пропуск запрещён)."""
...
def main() -> int:
    ...
    # ⚠ ЕДИНСТВЕННЫЙ фильтр «это коммит или нет», и он ЗДЕСЬ, а не в настройках.
    # Прежняя редакция комментария ссылалась на поле `if` в .claude/settings.json —
    # его там нет с 2026-08-17, когда запись с этим полем перестала исполняться
    # средой и хук умер бесшумно, неизвестно с какого числа. Класс отказа не наш
    # и не единичный: в трекере anthropics/claude-code он описан заявками #11394,
    # #11544 (хуки не грузятся из settings.json), #6305, #40506 (не срабатывают
    # в `claude -p`). Вывод: фильтр по содержимому команды держит СКРИПТ, потому
    # что скрипт либо запустился, либо нет, и это видно по коду возврата.
    ...
    # ⚠ ХВОСТ НЕ ПОКАЗЫВАЕТСЯ, и это правка дефекта, а не вкус. CLAUDE.md, раздел
    # "Проверка": хвост вывода — не сводка. 2026-08-18 хвост от scripts/check.sh дал
    # три провала, а их было ЧЕТЫРЕ: каждый гейт печатает отказ своими словами
    # (`doc_refs` — "БИТАЯ ССЫЛКА", где слова "провал" нет вовсе), и в конец их никто
    # не собирает. Правило было записано в CLAUDE.md — и нарушалось ЗДЕСЬ, в приборе,
    # который блокирует коммит. Инверсия снимает вопрос: успех узнаётся по одной
    # приставке, всё прочее уцелевает независимо от формулировки отказа.
    lines = (done.stdout + done.stderr).splitlines()
    failed = [ln for ln in lines if not ln.startswith(OK_PREFIX)]
    shown = failed[:MAX_LINES]
    cut = len(failed) - len(shown)
    more = f"\n\n…и ещё {cut} строк отказа не показано (потолок {MAX_LINES})." if cut else ""
    body = "\n".join(shown) or "(гейты не напечатали ни строки — смотреть вручную)"
    _block(
        "КОММИТ ОСТАНОВЛЕН: гейты красные.\n"
        f"`bash {GATES}` вернул код {done.returncode}. "
        f"Строк не-успеха: {len(failed)} из {len(lines)}.\n\n{body}{more}\n\n"
