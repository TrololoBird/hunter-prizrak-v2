"""ГЕЙТ: обе ветви бинирования цены дают ОДИН И ТОТ ЖЕ бин. FOUNDATION.md §5, §10.3.

§5 требует, чтобы бины профиля были привязаны к `tickSize`. Требование выполнялось
дважды и по-разному: живой поток считал бин через `Decimal`, архив — через `float`-
деление с `floor`. Реальные цены сделок ВСЕГДА кратны шагу, то есть всегда ложатся ровно
