...
def _pair_structure(hi1: float, hi2: float, lo1: float, lo2: float) -> list[Bar]:
    """Ряд с ровно ЧЕТЫРЬМЯ точками границ в порядке хай-лой-хай-лой и выходом вверх.

    Цены точек — аргументы, чтобы одну и ту же геометрию можно было предъявить в РАЗНОМ
    ПОРЯДКЕ прихода: границу задаёт внутренний край пары, а какая из двух точек оказалась
    внешней — вопрос порядка, и стр. 18 отвечает на него по-разному.
    """
...
def c_puncture_needs_third_point() -> tuple[object, object]:
    ...
    Номер точки СКВОЗНОЙ, через обе стороны: на схеме стр. 18 сверху стоят 1, 3, 5, 7, 9,
    снизу — 2, 4, 6, 8, 10. Граница стороны — внутренний край её первых двух точек, значит
    внешняя из пары лежит за границей ВСЕГДА. Но под правило она попадает, только если
    пришла ВТОРОЙ: тогда её сквозной номер 3 или 4. Пришедшая первой имеет номер 1 или 2,
    и стр. 18 её не берёт.

    Геометрия в обоих рядах одна: верхняя граница 107, нижняя 93, внешние точки 108 и 92.
    Меняется ТОЛЬКО порядок прихода точек пары — и ответ обязан смениться вместе с ним.
    Это же и контроль: прибор, отвечающий одинаково на оба ряда, проверку не проходит.
    """
    outer_first = _pair_structure(108.0, 107.0, 92.0, 93.0)
    outer_second = _pair_structure(107.0, 108.0, 93.0, 92.0)
    got: list[object] = []
    for b in (outer_first, outer_second):
    ...
    return tuple(got), (
        (107.0, 93.0, None, None),      # внешние пришли первыми — стр. 18 не применяется
        (107.0, 93.0, 108.0, 92.0),     # внешние пришли вторыми — прокол идёт в стоп
    )
